...
*
* @package phpBB2
* @version $Id: rss.php,v 2.2.4 2005/11/23 21:15:00 chyduskam Exp $
* @copyright (c) 2004 Egor Naklonyaeff
...
$sql = "SELECT MAX(post_time) AS pt 
	FROM " . POSTS_TABLE;
...
$needlogin = ( isset($HTTP_GET_VARS['login']) || isset($HTTP_GET_VARS['uid'])) ? true : false;
...
else
{
	...
	$viewpost = real_path('viewtopic.'.$phpEx);
	$replypost = real_path('posting.'. $phpEx.'?mode=quote');
	$index = real_path('index.'. $phpEx);

	// Assemble URL components
	$index_url = real_path('');
	$viewpost_url = $viewpost;
	$replypost_url = $replypost;
	...
	if ($userdata['user_level'] <> ADMIN)
	{
		...
		{
			if ((!$is_auth[$forum_id]['auth_read']) || (strpos(",$unauthed," , ",$forum_id,")))
			{
				if ($needlogin) 
				{
			  	...
			  	{
					header('Location: ' . $index_url . 'rss.'.$phpEx.'?' . POST_FORUM_URL . '=' . $forum_id . (($no_limit) ? '&nolimit' : '') . (isset($HTTP_GET_VARS['atom']) ? '&atom' : '') . (isset($HTTP_GET_VARS['c']) ? '&c=' . $count : '') . (isset($HTTP_GET_VARS['t']) ? '&t=' . $topics_only : '') . (isset($HTTP_GET_VARS['styled']) ? '&styled' : '') . '&login');
	...
	//
	// SQL statement to fetch active posts of allowed forums
	//
	$sql_limit_by_http = '';
	...
	if (!$no_limit)
	{
		if (isset($HTTP_SERVER_VARS['HTTP_IF_MODIFIED_SINCE'])) 
		{
			...
			else if ($NotModifiedSince > $MaxRecordAge) 
			{
	...
	//
	// Assign static variables to template
	//
	...
	else
	{
		...
		foreach ($allposts as $post)
		{
			...
			if ( $post['user_id'] != -1 )
			{
				 $author = '<a href="' . $index_url . 'profile.' . $phpEx . '?mode=viewprofile&' . POST_USERS_URL . '=' . $post['user_id'] . '" target="_blank">' . $author . '</a>';
			}
			else
			{
		...
		if ($user_id != ANONYMOUS && UPDATE_VIEW_COUNT)
		{
			...
			foreach ($SeenTopics as $topic_id=>$tcount)
			{
				...
				if (defined('TOPIC_VIEW_TABLE') && AUTO_WVT_MOD)
				{
	...
	// BEGIN Cache Mod
        //
	if (($user_id == ANONYMOUS) && CACHE_TO_FILE && ($cache_root!='') && empty($HTTP_GET_VARS) && !isset($HTTP_SERVER_VARS['HTTP_IF_MODIFIED_SINCE']) && !(AUTOSTYLED && strpos($useragent, 'MSIE')))
	{
		ob_start();
