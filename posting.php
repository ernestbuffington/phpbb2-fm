...
*
* @package phpBB2
* @version $Id: posting.php,v 1.159.2.28 2006/01/28 14:56:51 grahamje Exp $
...
/*
	...
	a relevant message such as this in posting.php ... failure to do so 
...
$params = array('submit' => 'post', 'preview' => 'preview', 'delete' => 'delete', 'poll_delete' => 'poll_delete', 'poll_add' => 'add_poll_option', 'poll_edit' => 'edit_poll_option', 'mode' => 'mode', 'back_to_calendar' => 'back_to_calendar');

$post_keys = array_keys($HTTP_POST_VARS);
$get_keys = array_keys($HTTP_GET_VARS);

for ($i = 0; $i < sizeof($HTTP_POST_VARS); $i++)
{
	if (substr($post_keys[$i], 0, 10) == "event_day(")
	{
		sscanf($post_keys[$i], "event_day(%[^[)]]", $cal_date);
		break;
	}
}
for ($i = 0; $i < sizeof($HTTP_GET_VARS); $i++)
{
	if (substr($get_keys[$i], 0, 10) == "event_day(")
	{
		sscanf($get_keys[$i], "event_day(%[^[)]] )", $cal_date);
		break;
	}
}
$HTTP_GET_VARS["cal_date"] = $cal_date;
...
$locktopic = ( !empty($HTTP_POST_VARS['locktopic']) ) ? htmlspecialchars($HTTP_POST_VARS['locktopic']) : 0;
$msg_icon_checked = $msg_icon = ($HTTP_POST_VARS['msg_icon']) ? intval($HTTP_POST_VARS['msg_icon']) : 0; 
...
{
	message_die(GENERAL_MESSAGE, $lang['force_email_info_message'] . '<br /><br />' . sprintf($lang['Click_update_email'], '<a href="' . append_sid('profile.'.$phpEx.'?mode=editprofile&amp;ucp=reg_info') . '">', '</a>')); 
}
...
if ( $userdata['user_level'] < 1 )
{
	if ( $mode == 'reply' )
	{
		...
		if ( $row['user_id'] == $userdata['user_id'] )
		{
			...
			$forum_redirect = '<meta http-equiv="refresh" content="3;url=' . append_sid("viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id") . '">';
			
			$message = $lang['tk_userkicked_topic'] . '<a href="' . append_sid("privmsg.$phpEx?mode=post&amp;" . POST_USERS_URL . "=$tk_kicker_id") . '">' . $kicker_username . '</a>' . $lang['tk_userkicked_contact'] . $forum_redirect;
...
switch ( $mode )
{
	...
	case 'vote':
		...
		$sql = "SELECT f.*, t.topic_status, t.topic_title, t.topic_type, t.topic_password  
	...
	case 'poll_delete':
		...
		$select_sql = (!$submit) ? ', t.topic_title, p.enable_bbcode, p.enable_html, p.enable_smilies, p.enable_sig, p.post_username, p.post_icon, p.post_time, pt.post_subject, pt.post_text, pt.bbcode_uid, u.username, u.user_id, u.user_sig, u.user_sig_bbcode_uid' : '';
		$from_sql = (!$submit) ? ", " . POSTS_TEXT_TABLE . " pt, " . USERS_TABLE . " u" : '';
		$where_sql = (!$submit) ? "AND pt.post_id = p.post_id AND u.user_id = p.poster_id" : '';
		$sql = "SELECT f.*, t.topic_id, t.topic_status, t.topic_type, t.topic_first_post_id, t.topic_last_post_id, t.topic_vote, t.topic_password, p.post_id, p.poster_id" . $select_sql . " 
...
if ( ($result = $db->sql_query($sql)) && ($post_info = $db->sql_fetchrow($result)) )
{
	...
	if ($is_auth['auth_mod'])
	{
		if ($locktopic)
		{
			...
			{
				$locktopic = ($post_info['topic_status'] == TOPIC_LOCKED) ? 2 : 1;
			}
		}
	}
	else
	...
	if ( $userdata['user_level'] < 1 )
	{
		if ( $mode == 'quote' || $mode == 'editpost' )
		{
			...
			if ( $row['user_id'] == $userdata['user_id'] )
			{
				...
				$message = $lang['tk_userkicked_topic'] . '<a href="' . append_sid("privmsg.$phpEx?mode=post&amp" . POST_USERS_URL . "=$tk_kicker_id") . '">' . $kicker_username . '</a>' . $lang['tk_userkicked_contact'] . $forum_redirect;
	...
	if( $userdata['user_level'] != ADMIN && ($mode == 'editpost' || $mode == 'delete' || $mode == 'poll_delete') && $post_info['topic_first_post_id'] == $post_id && $post_info['topic_password'])
	{
		message_die(GENERAL_MESSAGE, $lang['Not_auth_edit_post'] . '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . append_sid("viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id") . '">', '</a>'));
	}
	...
	if ( $mode == 'editpost' || $mode == 'delete' || $mode == 'poll_delete' )
	{
		...
		if ( $post_data['first_post'] && $post_data['has_poll'] )
		{
			...
			if ( $row = $db->sql_fetchrow($result) )
			{
				...
				$ajaxed_poll_count = sizeof($poll_options);
			}
...
// If user is not allowed to POST events in a calendar forum, check if they can SUGGEST them
if ( !$is_auth[$is_auth_type] )
{
	if ($is_auth_type == 'auth_post' && mycal_forum_check($forum_id))  $is_auth_type = 'auth_suggest_event';
}
...
// Journal
//
$journal_exit_msg = "<br />" . sprintf($lang['Click_return_forum'], "<a href=\"" . append_sid("viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id") . "\">", "</a>") . "<br /><br />" . sprintf($lang['Click_return_index'], "<a href=\"" . append_sid("index.$phpEx") . "\">", "</a>") . "<br />"; 

if ( ($forum_id == $board_config['journal_forum_id']) && ($mode == 'reply' || $mode == 'quote') ) 
{ 
	...
	if ( ($topic_poster != $userdata['user_id']) && ($userdata['user_level'] != ADMIN && $userdata['user_level'] != LESS_ADMIN && $userdata['user_level'] != MOD) )  
	{ 
		message_die(GENERAL_MESSAGE, $lang['Journal_reply_message'] . $journal_exit_msg); 
	} 
} 

if ( ($forum_id == $board_config['journal_forum_id']) && ($mode == 'newtopic') )
{ 
	$sql = "SELECT * 
		FROM " . TOPICS_TABLE . " 
		WHERE forum_id = " . $forum_id . " 
...
if ( ( $delete || $poll_delete || $mode == 'delete' ) && !$confirm )
{
	...
	// Confirm deletion
	//
	$s_hidden_fields = '<input type="hidden" name="' . POST_POST_URL . '" value="' . $post_id . '" /><input type="hidden" name="back_to_calendar" value="' . $back_to_calendar . '" />';
...
else if ( $mode == 'thank' ) 
{
	...
	if ( !($userdata['session_logged_in']) )
	{
		$message =  $lang['thanks_not_logged'] . '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . append_sid("viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid('index.'.$phpEx) . '">', '</a>');
	...
	{
		$message =  $lang['t_starter'] . '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . append_sid("viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid('index.'.$phpEx) . '">', '</a>');

		message_die(GENERAL_MESSAGE, $message);
	}

	// Check if user has thanked before
	$sql = "SELECT topic_id
		FROM " . THANKS_TABLE . " 
		WHERE topic_id = $topic_id
			AND user_id = " . $userdata['user_id'];
	...
	$message .=  '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . append_sid("viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid('index.'.$phpEx) . '">', '</a>');
...
else if ( $mode == 'vote' )
{
	...
	else if (($HTTP_GET_VARS['vote']) && ($HTTP_GET_VARS['vote'] == -1) && ($board_config['null_vote'] == 1))
	{
		$sql = "SELECT vd.vote_id    
...
else if ( $submit || $confirm )
{
	...
	switch ( $mode )
	{
		case 'editpost':
			...
			$max_vote = ( isset($HTTP_POST_VARS['max_vote']) && $is_auth['auth_pollcreate'] ) ? $HTTP_POST_VARS['max_vote'] : '';
			$voted_vote = ( isset($HTTP_POST_VARS['voted_vote']) && $is_auth['auth_pollcreate'] ) ? $HTTP_POST_VARS['voted_vote'] : '';
			$bbcode_uid = '';
		
			prepare_post($mode, $post_data, $bbcode_on, $html_on, $smilies_on, $error_msg, $username, $bbcode_uid, $subject, $message, $poll_title, $poll_options, $poll_length, $topic_password);
			...
			if ( $error_msg == '' )
			{
				...
				submit_post($mode, $post_data, $post_index, $return_message, $return_meta, $forum_id, $topic_id, $post_id, $poll_id, $topic_type, $bbcode_on, $html_on, $smilies_on, $attach_sig, $urgent_post, $bbcode_uid, $username, $subject, $message, $poll_title, $poll_options, $poll_length, $topic_password, $msg_icon, $voted_vote, $locktopic);
						
				// Log modertaor event
				if ( $board_config['enable_mod_logger'] && $is_auth['auth_mod'] )
				{
		...
		case 'reply':
			...
			if ( $board_config['enable_confirm_posting'] && !$userdata['session_logged_in'] )
			{
			...
			if ( $error_msg == '' )
			{
				...
				submit_post($mode, $post_data, $post_index, $return_message, $return_meta, $forum_id, $topic_id, $post_id, $poll_id, $topic_type, $bbcode_on, $html_on, $smilies_on, $attach_sig, $urgent_post, $bbcode_uid, $username, $subject, $message, $poll_title, $poll_options, $poll_length, $topic_password, $msg_icon, $locktopic);
			}
			break;
	...
	if ( $error_msg == '' )
	{
		if ($locktopic > 0 && (($mode=='editpost' && !$post_data['first_post']) || $mode=='reply'))
		{
			$sql = "UPDATE " . TOPICS_TABLE . "
				SET topic_status = " . (($locktopic == 2) ? TOPIC_UNLOCKED : TOPIC_LOCKED) . " 
				WHERE topic_id = $topic_id 
		...
		if ($error_msg == '' && $mode != 'poll_delete' && $board_config['disable_topic_watching'])
		{
			// If this post is a new topic..
			if ($post_data['first_post'])
			{
				// Check out which ppl are watching this forum..
				$forum_notify_sql = "SELECT user_id 
			...
			$event_type_id = $HTTP_POST_VARS[event_type_id]; 
			mycal_submit_event($mode, $forum_id, $topic_id, $post_id, $event_type_id, ($is_auth_type == 'auth_suggest_event'));
		}
		...
		{
			$template->assign_vars(array(
				'META' => "<meta http-equiv='refresh' content='3;url=". append_sid($back_to_calendar) . "'>")
			);
		}
...
if( $refresh || isset($HTTP_POST_VARS['del_poll_option']) || $error_msg != '' )
{
	...
	{ 	
		...
		{
			$preview_message = $preview_message . '</span><br /><br />_________________<br />' . $user_sig;
		}
		...
		$template->assign_vars(array(
			...
			'CUSTOM_POST_COLOR' => ( $board_config['allow_custom_post_color'] && $userdata['user_custom_post_color'] && $userdata['user_id'] != ANONYMOUS ) ? $userdata['user_custom_post_color'] : '',
...
else
{
	...
	if ( $postreport )
	{
		$sql = 'SELECT topic_id 
			FROM ' . POSTS_TABLE . ' 
			WHERE post_id = "' . $postreport . '"';
		if( !($result = $db->sql_query($sql) )) 
		{
			message_die(GENERAL_ERROR, "Could not get post subject information", '', __LINE__, __FILE__, $sql);
		}
		...
		$sql = 'SELECT pt.post_subject 
			...
			WHERE p.topic_id = "' . $post_topic_id . '" 
				AND pt.post_id = p.post_id 
			ORDER BY p.post_time ASC LIMIT 1';
		if( !($result = $db->sql_query($sql) )) 
		{
			message_die(GENERAL_ERROR, "Could not get topic subject information", '', __LINE__, __FILE__, $sql);
		}
		$post_details = $db->sql_fetchrow($result);
	...
	if ( $mode == 'newtopic' )
	{
		...
		$poll_title = $poll_length = $message = $topic_password = '';
	}
	else if ( $mode == 'reply' )
	{
		$user_sig = ( $userdata['user_sig'] != '' ) ? $userdata['user_sig'] : '';
...
//
if ( $userdata['session_logged_in'] && $is_auth['auth_read'] && $board_config['disable_topic_watching'] )
{
	if ( $mode != 'editpost' || ( $mode == 'editpost' && $post_info['poster_id'] != ANONYMOUS ) )
	{
...
if ( $is_auth['auth_mod'] )
{
	$template->assign_block_vars('switch_locktopic_checkbox', array());
}
...
//
$confirm_image = '';
if( !$userdata['session_logged_in'] && (!empty($board_config['enable_confirm_posting'])) )
{
...
$ajaxed_preview = '';
if ( $board_config['AJAXed_status'] && $board_config['AJAXed_post_preview'] )
{
	include($phpbb_root_path . 'includes/sajax.'.$phpEx);
	$sajax->request_type = 'POST';
	$sajax->init('ajax.'.$phpEx);
	$sajax->export('preview_post');
	$sajax->handle_client_request();
	$sajax->show_javascript();
	$template->assign_block_vars('switch_ajax', array());
	$ajaxed_preview = ( $board_config['AJAXed_post_preview'] ) ? ' onClick="pp(document.post.message.value,document.post.subject.value); return false;"' : '';
}
...
// Topic password
//
if ( $show_password_box && $userdata['user_level'] == ADMIN && $post_info['topic_password'])
{
...
$template->assign_vars(array(
	...
	'AJAXED_POLL_OPTION_COUNT' => $ajaxed_poll_count,
	...
	'L_LOCK_TOPIC' => ( $post_info['topic_status'] == TOPIC_LOCKED ) ? $lang['Unlock_topic'] : $lang['Lock_topic'], 
	'L_URGENT_POST' => $lang['Urgent_post'],
	'L_CONFIRM_CODE_TITLE' => $lang['Confirm_code_posting_title'], 
	...
	'S_LOCK_TOPIC_CHECKED' => ( $locktopic ) ? 'checked="checked"' : '',
...
if( ( $mode == 'newtopic' || ( $mode == 'editpost' && $post_data['edit_poll']) ) && $is_auth['auth_pollcreate'] )
{
	$template->assign_vars(array(
		...
		'POLL_AJAX_JS2' => ' onClick="kf(); return false;"',
	...
	if( !empty($poll_options) )
	{
		while( list($option_id, $option_text) = each($poll_options) )
		{
			$template->assign_block_vars('poll_option_rows', array(
				'POLL_AJAX_JS' => ' onClick="kc(this); return false;"',
