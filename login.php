				...
				if( md5($password) == $row['user_password'] && $row['user_active'] )
				{
					...
					if( $session_id )
					{
						...
						if ( !$board_config['privmsg_disable'] && !$board_config['wpm_disable'] && !$row['user_totallogon'] )
						{
						    ...
						    if ( !$db->sql_query($sql) )
							{
								message_die(GENERAL_ERROR, 'Could not insert welcome private message sent info', '', __LINE__, __FILE__, $sql);
							}
							...
							$sql = "INSERT INTO " . PRIVMSGS_TEXT_TABLE . " (privmsgs_text_id, privmsgs_text) 
								VALUES ($privmsg_sent_id, '" . str_replace("\'", "''", addslashes(sprintf($register_pm, $row['username'], $board_config['sitename'], $board_config['sitename']))) . "')";
							if ( !$db->sql_query($sql) )
							{
								message_die(GENERAL_ERROR, 'Could not insert welcome private message sent text', '', __LINE__, __FILE__, $sql);
							}
						}	
