...
//
$sql = "SELECT u.user_id, u.username, u.user_level, u.user_session_time, u.user_session_page, u.user_allow_viewonline, s.session_logged_in, s.session_ip, s.session_start 
	...
	for($i = 0; $i < sizeof($onlinerow_reg); $i++)
	{
		if ( !inarray($onlinerow_reg[$i]['user_id'], $reg_userid_ary) )
		{
			...
			$template->assign_block_vars("reg_user_row", array(
				...
				"U_WHOIS_IP" => ($board_config['mods_viewips']) ? '<a href="http://network-tools.com/default.asp?prog=trace&amp;host=' . $reg_ip . '" class="genmed" target="_blank">' . $reg_ip . '</a>' : $lang['Hidden_email'], 
...
if( sizeof($onlinerow_guest) )
{
	...
	for($i = 0; $i < sizeof($onlinerow_guest); $i++)
	{
		...
		$template->assign_block_vars("guest_user_row", array(
			...
			"U_WHOIS_IP" => ($board_config['mods_viewips']) ? '<a href="http://network-tools.com/default.asp?prog=trace&amp;host=' . $guest_ip . '" class="genmed" target="_blank">' . $reg_ip . '</a>' : $lang['Hidden_email'], 
			"U_GUEST_IP_LOOKUP" => ($board_config['mods_viewips']) ? '[ <a href="' . append_sid($guest_ip_lookup) . '" class="gensmall">' . $lang['Poster'] . ' ?</a> ]' : '',
			"U_FORUM_LOCATION" => append_sid($location_url))
		);
	}
}
