...
// Module Activation
if ($board_config['enable_module_users'])
{
...
if( isset( $HTTP_GET_VARS['username'] ) )
{
	...
	attachment_quota_settings('user', $HTTP_POST_VARS['submit'], $mode);
		...
		$s_hidden_fields .= '<input type="hidden" name="returntoprofile" value="' . $return_to_profile .'" />';
			...
			// Birthday
			//
			$s_b_day = $lang['Day'] . '&nbsp;<select name="b_day" ' . (( $this_userdata['user_level'] == ADMIN || ( $this_userdata['user_level'] == MOD && $userdata['user_id'] != $this_userdata['user_id'] ) ) ? ' disabled="disabled"' : '') . '>
			...
			$s_b_md = $lang['Month'] . '&nbsp;<select name="b_md" ' . (( $this_userdata['user_level'] == ADMIN || ( $this_userdata['user_level'] == MOD && $userdata['user_id'] != $this_userdata['user_id'] ) ) ? ' disabled="disabled"' : '') . '> 
			<option value="0">--</option> 
			...
			$s_b_year = $lang['Year'] . '&nbsp;<input type="text" class="post" name="b_year" size="5" maxlength="4" value="' . $b_year . '"' . (( $this_userdata['user_level'] == ADMIN || ( $this_userdata['user_level'] == MOD && $userdata['user_id'] != $this_userdata['user_id'] ) ) ? ' disabled="disabled"' : '') . ' />';
			$i = 0;
			$s_birthday = '';
        ...
        if (!empty($custom_post_color))
		{
			...
			$flag_select = "<select name=\"user_flag\" onChange=\"document.images['user_flag'].src = '../images/flags/' + this.value;\" " . (( $this_userdata['user_level'] == ADMIN || ( $this_userdata['user_level'] == MOD && $userdata['user_id'] != $this_userdata['user_id'] ) ) ? ' disabled="disabled"' : '') . ">";
