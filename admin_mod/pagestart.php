...
{
	redirect(append_sid("login.$phpEx?redirect=admin_mod/index.$phpEx", true));
}
else if ( $userdata['user_level'] != MOD && $userdata['user_level'] != LESS_ADMIN && $userdata['user_level'] != ADMIN )
{
...
if ($board_config['admin_login'])
{
	...
	{
	   redirect(append_sid("login.$phpEx?redirect=admin_mod/index.$phpEx&admin=1", true));
	}
}
...
include('../admin/page_menu_mod.'.$phpEx);
