...
<table width="100%" cellpadding="2" cellspacing="2">
  <tr>
	<td class="nav"><a href="{U_INDEX}" class="nav">{L_INDEX}</a> -> <a href="{U_DOWNLOAD_HOME}" class="nav">{DOWNLOAD}</a><!-- BEGIN navlinks --> -> <a href="{navlinks.U_VIEW_CAT}" class="nav">{navlinks.CAT_NAME}</a><!-- END navlinks --> -> <a href="{U_FILE_NAME}" class="nav">{FILE_NAME}</a> -> {L_EMAIL}</td>
  </tr>
</table>

<table width="100%" cellpadding="4" cellspacing="1" class="forumline"><form action="{S_EMAIL_ACTION}" method="post" onSubmit="return checkForm(this)" name="post">
  <tr> 
	<th class="thHead" colspan="2">{L_EMAIL}</th>
  </tr>
  <tr> 
	<td class="row2" colspan="2"><span class="gensmall">{L_EMAILINFO}</span></td>
  </tr>
  <tr> 
  ...
  <tr> 
	...
	<td class="row2"><textarea cols="38" rows="10" name="message" class="post">{L_DEFAULTMAIL} {FILE_URL}</textarea></td>
  </tr>
  <tr> 
	<td class="catBottom" align="center" colspan="2">{S_HIDDEN_FIELDS}<input type="hidden" name="action" value="email"><input type="hidden" name="file_id" value="{ID}"><input class="liteoption" type="submit" name="submit" value="{L_SEMAIL}"></td>
  </tr>
</form></table>
