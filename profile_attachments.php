...
*
* @package phpBB2
* @version $Id: profile_attachments.php,v 1.4 2006/09/06 14:26:29 acydburn Exp $
...
if (!file_exists($phpbb_root_path . 'language/lang_' . $language . '/lang_admin_attach.'.$phpEx)) { $language = $attach_config['board_lang']; } include($phpbb_root_path . 'language/lang_' . $language . '/lang_admin_attach.' . $phpEx);
...
else if ($delete && sizeof($delete_id_list) > 0)
{
	...
	$template->assign_vars(array(
		...
		'S_CONFIRM_ACTION' => append_sid($phpbb_root_path . 'profile_attachments.' . $phpEx),
...
$template->set_filenames(array(
	'body' => 'profile_attachments_body.tpl')
);
...
$template->assign_vars(array(
	...
	'S_MODE_ACTION'	=> append_sid($phpbb_root_path . 'profile_attachments.' . $phpEx),
			...
			if ($ids[$j]['post_id'] != 0)
			{
				$sql = "SELECT t.topic_title
					FROM " . TOPICS_TABLE . " t, " . POSTS_TABLE . " p
						WHERE p.post_id = " . (int) $ids[$j]['post_id'] . " 
				...
				$post_titles[] = '<a href="' . $view_topic . '" class="gensmall">' . $post_title . '</a>';
			}
			else
			{
				$desc = '';
...
{
	$template->assign_block_vars('noattachrow', array(
		'L_NONE' => $lang['No_user_files'])
	);
}
...
{
	$pagination = generate_pagination($phpbb_root_path . 'profile_attachments.' . $phpEx . '?mode=' . $mode . '&amp;order=' . $sort_order . '&amp;' . POST_USERS_URL . '=' . $user_id . '&amp;sid=' . $userdata['session_id'], $total_rows, $board_config['topics_per_page'], $start).'&nbsp;';
