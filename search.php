...
*
* @package phpBB2
* @version $Id: search.php,v 1.72.2.16 2005/03/15 18:34:34 acydburn Exp $
...
{	
	message_die(GENERAL_MESSAGE, $lang['Search_disabled']);
}
...
{ 
	...
	{ 
		$search_author = ( isset($HTTP_POST_VARS['username']) ) ? $HTTP_POST_VARS['username'] : $HTTP_GET_VARS['username'];
	} 
	else 
	{
		$search_author = ''; 
...
$submit = ( isset($HTTP_POST_VARS['view']) || isset($HTTP_GET_VARS['view']) ) ? TRUE : 0; 
...
$sort_by_types = array($lang['Sort_Time'], $lang['Post_subject'], $lang['Sort_Topic_Title'], $lang['Author'], $lang['Forum']);
...
else if ( $search_keywords != '' || $search_author != '' || $search_id )
{
	...
	//
	if ( $search_id == 'newposts' || $search_id == 'egosearch' || $search_id == 'unanswered' || $search_keywords != '' || $search_author != ''  || $search_id == 'mini_cal' || $search_id == 'mini_cal_events' )
	{
		//
		// Flood control
		//
		...
		if ( $search_id == 'newposts' || $search_id == 'egosearch' || ( $search_author != '' && $search_keywords == '' ) || $search_id == 'mini_cal' || $search_id == 'mini_cal_events' )
		{
	        ...
	        else if ( $search_id == 'mini_cal_events' )
	    	{
		    	...
		    	include_once($phpbb_root_path . 'mods/calendar/mini_cal_' . MYCAL. '.'.$phpEx);

		        $sql = getMiniCalSearchSql('mini_cal_events', $search_date);
			...
			else
			{
				...
				{
					message_die(GENERAL_MESSAGE, $lang['No_search_match'] . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid('index.'.$phpEx) . '">', '</a>'));
				}
...
//			unset($stripped_keywords);
				...
				switch ( $split_search[$i] )
				{
					...
					default:
						...
						else
						{
							...
							$sql = "SELECT pt.post_id FROM " . POSTS_TEXT_TABLE . " pt " . (($only_bluecards) ? ', ' . POSTS_TABLE . ' p ' : '') . "
		...
		{
			$starter_from_sql = $starter_where_sql = ''; 
	...
	if ( $search_results != '' )
	{
		...
		{
			$sql = "SELECT pt.post_text, pt.bbcode_uid, pt.post_subject, p.*, f.forum_id, f.forum_name, f.display_pic_alert, t.*, u.username, u.user_id, u.user_sig, u.user_sig_bbcode_uid, u.user_level  
		...
		{
			$sql = "SELECT pt.post_text, t.*, f.forum_id, f.forum_name, f.display_pic_alert,  u.username, u.user_level, u.user_id, u2.username AS user2, u2.user_id AS id2, u2.user_level AS user_level2, p.post_username, p2.post_username AS post_username2, p2.post_time 
		...
		{
			case 1:
				$sql .= ( $show_results == 'posts' ) ? 'pt.post_subject' : 't.topic_title, t.title_compl_infos';
				break;
			case 2:
				$sql .= 't.topic_title, t.title_compl_infos';
				break;
			case 3:
				$sql .= 'u.username';
				break;
		...
		for($i = 0; $i < sizeof($searchset); $i++)
		{
			...
			//
			$picture_alert = '';
			if ($searchset[$i]['display_pic_alert'])
			{
				$sql_text = "SELECT pt.post_text, pt.bbcode_uid
					FROM " . POSTS_TEXT_TABLE . " pt, " . POSTS_TABLE . " p
					WHERE pt.post_id = p.post_id
					AND p.topic_id = $topic_id";
				if( !$text_result = $db->sql_query($sql_text) )
				{
					message_die(GENERAL_ERROR, 'Could not retrieve posts information', '', __LINE__, __FILE__, $sql_text);
				}
			...
			if ( $show_results == 'posts' )
			{
				...
				$searchset[$i]['username'] = username_level_color($searchset[$i]['username'], $searchset[$i]['user_level'], $searchset[$i]['user_id']);
			...
			else
			{
				...
				$views = ( $board_config['enable_topic_view_users'] && $userdata['session_logged_in'] ) ? '<a href="javascript:who_viewed(' . $topic_id . ')">' . $searchset[$i]['topic_views'] . '</a>' : $searchset[$i]['topic_views'];
				...
				$searchset[$i]['user2'] = username_level_color($searchset[$i]['user2'], $searchset[$i]['user_level2'], $searchset[$i]['id2']);
				$last_post_author = ( $searchset[$i]['id2'] == ANONYMOUS ) ? ( ($searchset[$i]['post_username2'] != '' ) ? $searchset[$i]['post_username2'] . ' ' : $lang['Guest'] . ' ' ) : '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '='  . $searchset[$i]['id2']) . '" class="gensmall">' . $searchset[$i]['user2'] . '</a>';
...
if ( $board_config['AJAXed_status'] && $board_config['AJAXed_user_list'] )
{
...
$template->assign_vars(array(
	...
	'L_SEARCH_KEYWORDS_EXPLAIN' => $lang['Search_keywords_explain'] . ' ' . $lang['Search_author_explain'], 
	...
	'AJAXED_USER_LIST_1' => ( $board_config['AJAXed_status'] && $board_config['AJAXed_user_list'] ) ? ' onkeyup="ub(this.value,1);" autocomplete="off"' : '',
	'AJAXED_USER_LIST_2' => ( $board_config['AJAXed_status'] && $board_config['AJAXed_user_list'] ) ? '<br /><div id="user_list" style="position:absolute"></div>' : '',
