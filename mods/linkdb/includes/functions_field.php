...
class custom_field
{
			...
			if(isset($this->field_data_rowset[$file_id]))
			{
				foreach($this->field_data_rowset[$file_id] as $field_id => $data)
				{
					if(!empty($data['data']))
					{
						...
						//if ( count($this->field_data_rowset[$file_id]) > 1 ) $field_data .= ',';
						if($serach_LinkDB)
						{
							$template->assign_block_vars('searchresults.custom_field', array(
							'CUSTOM_NAME' => $this->field_rowset[$field_id]['custom_name'],
							'DATA' => $field_data));
						}
						else
						{
							$template->assign_block_vars('FILELIST.file_rows.custom_field', array(
							'CUSTOM_NAME' => $this->field_rowset[$field_id]['custom_name'],
							'DATA' => $field_data));
						}
					}
	...
	function update_add_field($field_type, $field_id = false)
	{
		...
		if(!$field_id)
		{
			$sql = "INSERT INTO " . LINK_CUSTOM_TABLE . " (custom_name, custom_description, data, regex, field_type)
	...
	function delete_field($field_id)
	{
		...
		$sql = "DELETE FROM " . LINK_CUSTOM_TABLE . " 
			WHERE custom_id = '$field_id'";
	...
	function file_update_data($file_id)
	{
		...
		if(!empty($field))
		{
			foreach($field as $field_id => $field_data)
			{
				...
				if(!empty($data))
				{
					$sql = "INSERT INTO " . LINK_CUSTOM_DATA_TABLE . " (customdata_file, customdata_custom, data) 
