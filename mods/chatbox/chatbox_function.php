...
function write_msg($nick, $msg)
{
	...
	{
		write_msg(_CHATBOX_SYSTEM_MSG,"<span style='color: #FF0000'><b><i>$nick</b> joined chat on " . create_date($board_config['default_dateformat'], time(), $board_config['board_timezone']) . "</i></span>");
		user_join($nick,"offline");
	}

	if (($nick != _CHATBOX_SYSTEM_MSG) && (isuser_active($nick) == "away"))
	{
		write_msg(_CHATBOX_SYSTEM_MSG, "<span style='color: red'><i><b>$nick</b> is online again</i></span>");
	    user_active($nick);
	}
...
function get_msg($start,$number,$dir)
{
	...
	$sql = "SELECT name,msg,timestamp FROM " . $table_chatbox_name . " ORDER BY timestamp LIMIT ". $start . ", " . $number;
...
function drop_user($usernick)
{
	...
	if ($lastactive != "")
	{
		...
		write_msg(_CHATBOX_SYSTEM_MSG, "<span style='color: red'><i><b>" . $usernick . "</b> logged out on " . create_date($board_config['default_dateformat'], time(), $board_config['board_timezone'])."</i></span>");
	}
}
