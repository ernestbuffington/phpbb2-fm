...
*
* @package chatbox
* @version $Id: messenger_view.php,v 1.18b 2002/8/03, 20:06:21 hnt Exp $
...
<body class="view" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" onload="window.scrollTo(0,99999);" link="#006699">
...
{
	add_msg($HTTP_POST_VARS['nick'], $HTTP_POST_VARS['sent'], $HTTP_POST_VARS['color']);
}

if (isset($HTTP_POST_VARS['nick']) && isset($HTTP_POST_VARS['url']))
{
	add_url($HTTP_POST_VARS['nick'], $HTTP_POST_VARS['url']);
}
...
function show_msg()
{
	...
	$colour = 0; // used to change the background colours
...
function add_msg($nick, $msg, $clor) // This function processes the submission from user - Smartor
{
	global $chatbox_config, $userdata;
	...
	if ($error == FALSE)
	{
		write_msg($nick, "<font color='$clor'>$msg</font>");
		user_active($nick);
	}
}

function add_url($nick, $msg) // This function processes the submission from user - Smartor
{
	global $chatbox_config, $userdata;
	...
	if ($error == FALSE)
	{
		write_msg($nick,"<a target=_blank href=http://$msg>$msg</a>");
		user_active($nick);
	}
}
