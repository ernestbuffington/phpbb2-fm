...
*
* @package mini_cal
* @version $Id: mini_cal_common.php,v 2.0.4 2004 netclectic Exp $
* @copyright (c) 2003 netclectic - Adrian Cockburn
...
function getMiniCalForumsAuth($userdata)
{
    ...
    $mini_cal_auth = array();
    $mini_cal_auth['view'] = $mini_cal_auth['post'] = '';
...
{
	global $template, $db, $phpEx, $lang, $phpbb_root_path;
    ...
    $sql = "SELECT 
        	...
        	t.topic_title, t.title_pos, t.title_compl_infos, t.title_compl_color
        ...
        if ( $db->sql_numrows($result) > 0 )
        {
       	    ...
       	    while ($row = $db->sql_fetchrow($result))
            {
				...
				if ($board_config['enable_quick_titles'])
				{
					if ( $row['title_pos'] )
					{
						$topic_title = (empty($row['title_compl_infos'])) ? $topic_title : $topic_title . '<br /><span style="color: #' . $row['title_compl_color'] . '">' . $row['title_compl_infos'] . '</span>';
					}
					else
					{
						$topic_title = (empty($row['title_compl_infos'])) ? $topic_title : '<span style="color: #' . $row['title_compl_color'] . '">' . $row['title_compl_infos'] . '</span><br />' . $topic_title;
					}		
				}
				
       			$template->assign_block_vars('mini_cal_events', array(
					'MINI_CAL_EVENT_DATE' => $cal_date,
                    'S_MINI_CAL_EVENT' => $topic_title, 
