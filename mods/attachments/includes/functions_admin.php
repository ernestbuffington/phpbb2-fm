...
function get_formatted_dirsize()
{
	...
	if (!intval($attach_config['allow_ftp_upload']))
	{
		if ($dirname = @opendir($upload_dir))
		{
			...
			{
				if ($file != 'Thumbs.db' && $file != 'index.html' && $file != 'index.htm' && $file != 'index.php' && $file != '.htaccess' && !is_dir($upload_dir . '/' . $file) && !is_link($upload_dir . '/' . $file))
				{
	...
	else
	{
		...
		for ($i = 0; $i < count($file_listing); $i++)
		{
			...
			if ($dirinfo[0] != 1 && $dirinfo[4] != 'index.php' && $dirinfo[4] != 'index.html' && $dirinfo[4] != 'index.htm' && $dirinfo[4] != 'Thumbs.db' && $dirinfo[4] != '.htaccess')
			{
