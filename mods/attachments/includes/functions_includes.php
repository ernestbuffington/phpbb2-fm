...
function attachment_quota_settings($admin_mode, $submit = false, $mode)
{
	...
	include_once($phpbb_root_path . 'mods/attachments/includes/functions_selects.' . $phpEx);
	include_once($phpbb_root_path . 'mods/attachments/includes/functions_admin.' . $phpEx);
...
function display_upload_attach_box_limits($user_id, $group_id = 0)
{
	...
	$template->assign_vars(array(
		'L_UACP' => $lang['UACP'],
		'L_UPLOAD_QUOTA' => $lang['Upload_quota'],
		'U_UACP' => $phpbb_root_path . 'profile_attachments.'.$phpEx.'?' . POST_USERS_URL . '=' . $user_id . '&amp;sid=' . $userdata['session_id'],
