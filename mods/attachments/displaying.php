...
{
	global $attach_config, $is_auth, $images;

	if (intval($switch_attachment) == 0 || (!($is_auth['auth_download'] && $is_auth['auth_view'])) || intval($attach_config['disable_mod']))
	{
		return '';
	}

	$image = '<img src="' . $images['attach_topic_icon'] . '" alt="" title="" /> ';
...
{
	global $attach_config, $userdata, $images;

	$auth = ( $userdata['user_level'] == ADMIN ) ? 1 : intval($attach_config['allow_pm_attach']);
	
	if (!attachment_exists_db($privmsg_id, PAGE_PRIVMSGS) || !$auth || intval($attach_config['disable_mod']))
	{
		return '';
	}
...
{
	global $attach_config, $is_auth, $allowed_extensions, $lang, $userdata, $display_categories, $upload_dir, $upload_icons, $template, $db, $theme, $images;
		...
		for ($i = 0, $size = sizeof($attachment_list); $i < $size; $i++)
		{
			...
			if (!$denied)
			{
				...
				if ($link)
				{
					...
					if ($upload_icons[$extension] == '')
					{
						$upload_image = '<img src="' . $images['attach_topic_icon'] . '" alt="" title="" />';
					}
					else if (trim($upload_icons[$extension]) != '')
					{
						$upload_image = '<img src="' . $upload_icons[$extension] . '" alt="" title="" />';
					}
...
{
	global $template, $upload_dir, $userdata, $allowed_extensions, $display_categories, $download_modes, $db, $lang, $phpEx, $attachments, $upload_icons, $attach_config, $forum_topic_data, $images;
	...
	for ($i = 0; $i < $num_attachments; $i++)
	{
		...
		{
			$upload_image = '<img src="' . $upload_icons[$attachments['_' . $post_id][$i]['extension']] . '" alt="" title="" />';
		}
		...
		if (!$denied || $userdata['user_level'] == ADMIN)
		{
			...
			if ( $forum_topic_data['image_ever_thumb'] == TRUE && (intval($display_categories[$attachments['_' . $post_id][$i]['extension']]) == IMAGE_CAT) )
			{
				$link = TRUE;
				$image = '';
				$thumbnail = '';
