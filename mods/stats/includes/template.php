...
*
* @package stats
* @version $Id: template.php,v 4.2.8 2003/03/16 19:38:27 acydburn Exp $
...
class Stats_template {
	...
	function set_template($stats_template = '')
	{
		...
		$this->root = $phpbb_root_path . 'templates/' . $stats_template;
	...
	function compile_return_load(&$_str, &$handle)
	{
		...
		if (file_exists($filename) && @filemtime($filename) >= @filemtime($this->files[$handle]))
		{
			...
			$filename = $this->cachedir . 'decompiled.'.$phpEx.'.html';
			$fp = @fopen($filename, 'wt');
			fwrite($fp, $_str);
			@fclose($fp);

			$this->compiled_data = '';
			include($this->cachedir . 'decompiled.'.$phpEx.'.html');
