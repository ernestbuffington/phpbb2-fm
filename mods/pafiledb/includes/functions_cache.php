...
class acm
{
	...
	{
		global $phpEx;
		@include($this->cache_dir . 'config_pafiledb.' . $phpEx);
	}
	...
	function save() 
	{
		...
		global $phpEx;
		$file = '<?php if (!defined(\'IN_PHPBB\')) { die(\'Hacking attempt\'); } $this->vars=' . $this->format_array($this->vars) . ";\n\$this->vars_ts=" . $this->format_array($this->vars_ts) . ' ?>';

		if ($fp = @fopen($this->cache_dir . 'config_pafiledb.' . $phpEx, 'wb'))
		{
			@flock($fp, LOCK_EX);
			fwrite($fp, $file);
			@flock($fp, LOCK_UN);
	...
	function tidy($expire_time = 0)
	{
		...
		if (file_exists($this->cache_dir . 'config_pafiledb.' . $phpEx))
		{
