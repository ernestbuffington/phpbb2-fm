...
class pafiledb_public extends pafiledb
{
	...
	function module($module_name)
	{
		if (!class_exists('pafiledb_' . $module_name))
		{
			...
			require_once($phpbb_root_path . 'mods/pafiledb/modules/pa_' . $module_name . '.'.$phpEx);
...
// Mx Addon
	...
	function category_display($cat_id = PA_ROOT_CAT)
	{
		...
		if($this->cat_empty())
		{
			...
			message_die(GENERAL_MESSAGE, $lang['No_pafiledb']);
		}
			...
			foreach($this->subcat_rowset[$cat_id] as $subcat_id => $subcat_row)
			{
				if( ($subcat_row['cat_allow_file'] == PA_CAT_ALLOW_FILE) )
				{
					...
					if(!empty($last_file_info['file_id']) && $this->auth[$subcat_id]['auth_read'])
					{
						...
						$last_file = '<a href="' . append_sid('dload.php?action=file&file_id=' . $last_file_info['file_id']) . '" alt="' . stripslashes($last_file_info['file_name']) . '" title="' . stripslashes($last_file_info['file_name']) . '">' . $last_file_name . '</a><br />';
						$last_file .= $last_file_time . ' <a href="' . append_sid('dload.php?action=file&file_id=' . $last_file_info['file_id']) . '"><img src="' . $images['icon_latest_reply'] . '" alt="' . $lang['View_latest_file'] . '" title="' . $lang['View_latest_file'] . '" /></a>';
					}
					else
					{
	...
	function display_files($sort_method, $sort_order, $start, $show_file_message, $cat_id = false)
	{
		...
		global $images, $lang, $phpEx, $pafiledb_functions, $phpbb_root_path, $theme;
		...
		for ($i = 0; $i < count($file_rowset); $i++) 
		{
			...
			if($file_rowset[$i]['file_pin'] != FILE_PINNED)
			{
				...
				{
					$posticon = $phpbb_root_path . 'images/spacer.gif';
				} 
				else 
				{
			...
			$pafiledb_template->assign_block_vars("file_rows", array(
				...
				'FILE_SCREENSHOT_URL' => $phpbb_root_path . 'templates/' . $theme['template_name'] . '/images/lwin.gif',
