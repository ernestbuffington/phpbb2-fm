...
class pafiledb_post_comment extends pafiledb_public
{
	function main($action)
	{
		...
		include_once($phpbb_root_path . 'mods/pafiledb/includes/functions_comment.'.$phpEx);
		...
		if(!$submit)
		{
			...
			$smilies_status = ( $userdata['user_allowsmile'] && $pafiledb_config['allow_smilies']  ) ? $lang['Smilies_are_ON'] : (($board_config['smilie_removal1']) ? $lang['Smilies_are_REMOVED'] : $lang['Smilies_are_OFF']);
	...
	'L_HIGHLIGHT_TEXT' => $lang['Highlight_text'],
			...
			{
				$pafiledb_template->assign_block_vars ('font_styles', array(
					'L_FONTNAME' => $font)
				);
			}
			...
			if($preview)
			{
				...
				else
				{
					...
					{
						$comments_text = smilies_code_removal($comments_text);
					}
				}
