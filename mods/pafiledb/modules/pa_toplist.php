		...
		switch ($mode)
		{
			case 'newest':
				...
				if(empty($selected_date))
				{
					for($j = 0; $j <= $days - 1; $j++)
					{
						...
						$pafiledb_template->assign_block_vars('files_date', array(
							...
							'DATES' => create_date($board_config['default_dateformat'], $day_time, $board_config['board_timezone']),
				...
				else
				{
					...
					if(!empty($file_ids))
					{
						$sql = "SELECT f1.*, AVG(r.rate_point) AS rating, COUNT(r.votes_file) AS total_votes, u.username, u.user_level, c.cat_id, c.cat_name, COUNT(cm.comments_id) AS total_comments
							FROM " . PA_FILES_TABLE . " AS f1
								LEFT JOIN " . PA_CATEGORY_TABLE . " AS c on f1.file_catid = c.cat_id
					...
					for ($i = 0; $i < count($file_rowset); $i++) 
					{
						...
						if ($file_rowset[$i]['file_pin'] != FILE_PINNED)
						{
							...
							{
								$posticon = '<img src="' . ICONS_DIR . $file_rowset[$i]['file_posticon'] . '" alt="" title="" />';
							}
						}
						else
						{
							$posticon = '<img src="' . $images['folder_sticky'] . '" alt="" title="" />';
						}

						$poster = ( $file_rowset[$i]['user_id'] != ANONYMOUS ) ? '<a href="' . append_sid('profile.'.$phpEx.'?mode=viewprofile&amp;' . POST_USERS_URL . '=' . $file_rowset[$i]['user_id']) . '" class="genmed">' : '';
						$poster .= ( $file_rowset[$i]['user_id'] != ANONYMOUS ) ? usename_level_color($file_rowset[$i]['username'], $file_rowset[$i]['user_level'], $file_rowset[$i]['user_id']) : $lang['Guest'];
			...
			case 'rating':
				...
				if ($limit)
				{
					$sort_method = ($mode == 'downloads') ? 'f1.file_dls' : 'rating';
					$sql_limit = "LIMIT 0, $limit ";
					
					$sql = "SELECT f1.*, AVG(r.rate_point) AS rating, COUNT(r.votes_file) AS total_votes, u.username, u.user_level, c.cat_id, c.cat_name
						FROM " . PA_FILES_TABLE . " AS f1
							LEFT JOIN " . PA_CATEGORY_TABLE . " AS c ON f1.file_catid = c.cat_id
							LEFT JOIN " . PA_VOTES_TABLE . " AS r ON f1.file_id = r.votes_file
							LEFT JOIN ". USERS_TABLE ." AS u ON f1.user_id = u.user_id
				...
				for($i = 0; $i < count($searchset); $i++)
				{
					...
					//===================================================
					// Get the post icon for this file
					//===================================================
					if ($searchset[$i]['file_pin'] != FILE_PINNED)
					{
						...
						{
							$posticon = '<img src="' . ICONS_DIR . $searchset[$i]['file_posticon'] . '" alt="" title="" />';
						}
					}
					else
					{
					...
					$poster .= ( $searchset[$i]['user_id'] != ANONYMOUS ) ? username_level_color($searchset[$i]['username'], $searchset[$i]['user_level'], $searchset[$i]['user_id']) : $lang['Guest'];
