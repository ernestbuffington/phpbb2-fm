...
class pafiledb_file extends pafiledb_public
{
	function main($action)
	{
		...
		{
			case 'oracle':
				$sql = "SELECT f.*, AVG(r.rate_point) AS rating, COUNT(r.votes_file) AS total_votes, u.user_id, u.username, u.user_level, COUNT(c.comments_id) as total_comments
		...
		$file_poster = ( $file_data['user_id'] != ANONYMOUS ) ? '<a href="' . append_sid('profile.'.$phpEx.'?mode=viewprofile&amp;' . POST_USERS_URL . '=' . $file_data['user_id']) . '" class="gensmall">' : '';
		$file_poster .= ( $file_data['user_id'] != ANONYMOUS ) ? username_level_color($file_data['username'], $file_data['user_level'], $file_data['user_id']) : $lang['Guest'];
		...
		{
			include($phpbb_root_path . 'mods/pafiledb/includes/functions_comment.'.$phpEx);
