...
*
* @package phpBB2
* @version $Id: class_contact.php,v 1.0.0 2003/12/14 12:33:06 grahamje Exp $
...
class contact_list 
{
	...
	function get_list($type = 'buddy', $limit = 0, $start = 0, $order = 'ASC', $limit_max = 0)
	{
		...
		$sql = 'SELECT ' . $fields_sql . ', u.username, u.user_level 
			FROM ' . CONTACT_TABLE . ' c, ' . USERS_TABLE . ' u 
			WHERE u.user_id = ' . $field_check_sql . '
	...
	*/
	...
	function change_inform($method, $id_list, $type = '')
	{
		...
		$base_profile_url .= '/profile.' . $phpEx . '?mode=viewprofile&amp;' . POST_USERS_URL . '=';
		...
		for ($i = 0; $i < $num_of_ids; $i++)
		{
			...
			$user_profile_string = $base_profile_url . $userdata['user_id'] . '&amp;sid=' . $userdata['session_id'];
			...
			if ( $to_userdata['user_notify_pm'] && !empty($to_userdata['user_email']) && $to_userdata['user_active'] )
			{
				...
				$emailer->assign_vars(array(
					'PM_USERNAME' => $to_username, 
					'FROM' => $userdata['username'], 
					'PM_SUBJECT' => $lang['System_title'],
	...
	function update_alerts($mark_list, $users_list, $userid)
	{
		...
		if ( $mark_size && $user_size )
		{
			...
			message_die(GENERAL_MESSAGE, $lang['Alerts_updated'] . '<br /><br />' . sprintf($lang['Click_return_buddylist'], '<a href="' . append_sid('contact.'.$phpEx) . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid("index.$phpEx") . '">', '</a>'));
		}
		elseif ( $user_size )
		{
