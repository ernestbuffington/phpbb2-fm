...
if( $type != 'buddy' && $type != 'ignore' && $type != 'disallow' && $type != 'alert' )
{
	...
	message_die(GENERAL_MESSAGE, $message . $contact_list->append_msg . '<br /><br />' . sprintf($lang['Click_return_buddylist'], '<a href="' . append_sid('contact.'.$phpEx) . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid("index.$phpEx") . '">', '</a>'));
...
else
{
	...
	if( !empty($_REQUEST['list_to_add']) )
	{
		...
		$sql = 'SELECT user_id 
			FROM ' . USERS_TABLE . ' 
			WHERE username IN (' . $list_to_lookup . ')';
		if( !$result = $db->sql_query($sql) )
		{
	...
	else if ( $confirm )
	{
		if( $action == 'update' )
		{
			...
			if ( $mark_size )
			{
				...
				message_die(GENERAL_MESSAGE, $moved_msg . $contact_list->append_msg . '<br /><br />' . sprintf($lang['Click_return_buddylist'], '<a href="' . append_sid('contact.'.$phpEx) . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid("index.$phpEx") . '">', '</a>'));
			}
			else
			{
				$message = $lang['No_Contact_changes'];
		...
		elseif( $action == 'delete' )
		{
			...
			if ( $mark_size )
			{
				...
				message_die(GENERAL_MESSAGE, $lang['Removed_selected_users'] . $contact_list->append_msg . '<br /><br />' . sprintf($lang['Click_return_buddylist'], '<a href="' . append_sid('contact.'.$phpEx) . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid("index.$phpEx") . '">', '</a>'));
			}
			else
			{
				message_die(GENERAL_MESSAGE, $lang['No_Contact_changes'] . $contact_list->append_msg . '<br /><br />' . sprintf($lang['Click_return_buddylist'], '<a href="' . append_sid('contact.'.$phpEx) . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid("index.$phpEx") . '">', '</a>'));
			}
		}
		else
		{
			message_die(GENERAL_MESSAGE, $lang['Invalid_contact_action'] . $contact_list->append_msg . '<br /><br />' . sprintf($lang['Click_return_buddylist'], '<a href="' . append_sid('contact.'.$phpEx) . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid("index.$phpEx") . '">', '</a>'));
		}
	}
}
