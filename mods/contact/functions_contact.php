...
function popup_buddy_alert($offline, $online)
{
	...
	$offline_names = $online_names = '';
	$base_url = $phpbb_root_path . 'profile.' . $phpEx . '?mode=viewprofile&amp;' . POST_USERS_URL . '=';
	foreach($rows as $val)
	{
	...
	$offline_str = $online_str = '';
	if( $offline_ids != 0 )
	{
		$offline_str = ( sizeof($offline) >= 2 ) ? $lang['Buddies_offline'] . ': ' : $lang['Buddy_offline'] . ': ';
	}
	
	if( $online_ids != 0 )
	{
		$online_str = ( sizeof($online) >= 2 ) ? $lang['Buddies_online'] . ': ' : $lang['Buddy_online'] . ': ';
	}
