...
*
* @package contact
* @version $Id: contactcp_show.php,v 0.3.0 2003/12/23 23:23:15 acydburn Exp $
...
switch($type)
{
	...
	case 'buddy':
		...
		$alert_edit = sprintf('<a href="' . append_sid($phpbb_root_path . "contact.$phpEx?mode=show&type=alert") . '" class="gensmall">%s</a>', $lang['Edit_alerts']);
...
$template->assign_vars(array(
	...
	'NO_LIST' => $l_no_list,
	'L_CONTACT_MAN' => $lang['Edit_alerts'],
...
if( $num )
{
	...
	foreach($contact_list->$temp_type as $key=>$val)
	{
		...
		$template->assign_block_vars('list.list_row', array(
			...
			'USERNAME' => username_level_color($val['username'], $val['user_level'], $val['user_id']),
