...
*
* @package prillian
* @version $Id: functions_im.php,v 0.7.0 2003/12/23 23:27:19 thoul Exp $
...
{
	global $db, $prill_config, $lang, $append_msg, $theme, $template, $images, $userdata, $board_config;
	...
	else
	{
		...
		if ( !($im_data = $db->sql_fetchrow($result)) )
		{
			$sql = "INSERT INTO " . IM_PREFS_TABLE . " (user_id, themes_id) 
				VALUES ('" . $userdata['user_id'] . "', '" . $board_config['default_style'] . "')";
			if( !$result = $db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, 'Could not insert new user information im_prefs table.', '', __LINE__, __FILE__, $sql);
			}
		}
	...
	if( $style_setup )
	{
		...
		$theme = ( $board_config['override_user_style'] ) ? setup_style($prill_config['themes_id']) : setup_style($userdata['user_style']);
	}
...
function process_user_online($user, $is_on_im)
{
	...
	$style_color = $user_online_name = $user_online_link = '';
...
{
	global $db, $im_userdata, $template, $images, $prill_config, $lang, $phpEx, $phpbb_root_path, $mode, $userdata;
...
function auto_prill_check()
{
	...
	$sql = 'SELECT session_popup 
		FROM ' . IM_SESSIONS_TABLE . ' 
		WHERE session_user_id = ' . $im_userdata['user_id'];
	if( !$result = $db->sql_query($sql) )
	{
		return 0;
...
function delete_read_ims($user_id)
{
	...
	$sql = 'SELECT privmsgs_id 
		FROM ' . PRIVMSGS_TABLE . ' 
		WHERE privmsgs_type = ' . IM_READ_MAIL . ' 
	...
	$sql = 'UPDATE ' . IM_PREFS_TABLE . ' 
		SET read_ims = read_ims - ' . $num_rows . ' 
