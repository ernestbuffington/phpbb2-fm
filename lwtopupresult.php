...
*
* @package phpBB
* @version $Id: lwtopupresult.php,v 1.0.0.1 2004/10/29 17:49:33 acydburn Exp $
...
else 
{
	...
	while (!feof($fp)) 
	{
		...
		{
			if( $row['user_level'] == ADMIN	|| $row['user_level'] == LESS_ADMIN || $row['user_level'] == MOD )
			{
				fclose ($fp);
		...
		if((($flag == 1 || $flag == 2) && strcasecmp($payment_status, 'Completed') == 0) || $flag == 3)
		{
			...
			if( strcasecmp( trim($paypalaccount), trim($receiver_email)) != 0 || strcasecmp( trim($board_config['paypal_currency_code']), trim($mc_currency) ) != 0)
			{
				//do nothing
				$errorflag = 1;
			}
...
//						}
				...
				if($useringroup == 1)
				{
					$sql = "UPDATE " . USER_GROUP_TABLE . " 
						SET ug_active_date = " . time() . ", ug_expire_date = " . $expiretime . " 
						WHERE user_id = " . $user_id . " 
							AND group_id = " . $group_id;			
				}
				else
				{
					$sql = "INSERT INTO " . USER_GROUP_TABLE . " (user_id, group_id, user_pending, ug_active_date, ug_expire_date) 
						VALUES ($user_id, $group_id, 0, " . time() . ", " . $expiretime . ")";
				}
				if ( !($result = $db->sql_query($sql)) )
				{
					//do nothing
				...
				if( $row['user_expire_date'] == 0)
				{
					$sql = "UPDATE " . USERS_TABLE . " 
						SET user_active = 1, user_actkey='', user_actviate_date = " . time() . ", user_expire_date = " . $expiretime . " 
						WHERE user_id = " . $user_id;	
					if ( !($result = $db->sql_query($sql)) )
					{
						//do nothing
