...
include($phpbb_root_path . 'mods/pafiledb/includes/functions_field.'.$phpEx);
...
switch($mode)
{
	...
	case 'maintenance':
		...
		$s_hidden_fields .= '<input type="hidden" name="safety" value="1">';
		break;
	case 'mirrors':
		...
		if(isset($files_data['row_set']))
		{
			...
			foreach($files_data['row_set'] as $file_data)
			{
				...
				$template->assign_block_vars('file_mode.file_row', array(
					...
					'L_APPROVE' => ($file_data['file_approved']) ? '<img src="' . $phpbb_root_path . $images['acp_disable'] . '" alt="' . $lang['Unapprove'] . '" title="' . $lang['Unapprove'] . '" />' : '<img src="' . $phpbb_root_path . $images['acp_enable'] . '" alt="' . $lang['Approve'] . '" title="' . $lang['Approve'] . '" />')
				);
			}
		}
	}
...
elseif($mode == 'add' || $mode == 'edit' || $mirrors)
{
	if($mode == 'add')
	{
		$file_name = $file_desc = $file_long_desc = $file_author = $file_version = $file_website = $pin_checked_yes = $approved_checked_no = $file_ssurl = $ss_checked_yes = $file_url = '';
		...
		$pin_checked_no = $approved_checked_yes = $ss_checked_no = ' checked="checked"';
		$file_download = 0;
