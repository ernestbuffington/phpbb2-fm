...
*
* @package admin
* @version $Id: admin_faq_editor.php,v 0.8.6 2002/05/12 15:57:45 verteron Exp $
* @copyright (c) 2001 Verteron.net
...
if( !empty($setmodules) )
{
	...
	$module['Language_edit']['faq'] = $file . "?file=faq";
	$module['Language_edit']['bbcode'] = $file . "?file=bbcode";
	$module['Moderators']['moderator'] = $file . "?file=faq_moderator";
	return;
}
...
* @version $Id: lang_faq.php,v 1.4.2.3 2002/12/18 15:40:20 psotfx Exp $
...
if ($cancel)
{
	redirect('admin/' . append_sid("admin_faq_editor.$phpEx", true));
}
...
if( !file_exists($phpbb_root_path . 'language/lang_' . $language . '/lang_admin_faq_editor.'.$phpEx) ) { $language = 'english'; } include($phpbb_root_path . 'language/lang_' . $language . '/lang_admin_faq_editor.' . $phpEx);
...
if( !isset($HTTP_GET_VARS['language']) && !isset($HTTP_POST_VARS['language']) )
{
	$template->set_filenames(array(
		'body' => 'admin/lang_faq_select_lang_body.tpl')
	);

	$template->assign_vars(array(
		'L_LANGUAGE' => $lang['Spell_lang'],
		'LANGUAGE_SELECT' => language_select2($board_config['default_lang'], 'language', $phpbb_root_path.'language'),
...
if( isset($HTTP_GET_VARS['mode']) || isset($HTTP_POST_VARS['mode']) )
{
	...
	switch($mode)
	{
		...
		case 'block_edit':
			$template->set_filenames(array(
				"body" => "admin/lang_faq_block_body.tpl")
			);
		...
		case 'quest_new':
			$template->set_filenames(array(
				'body' => 'admin/lang_faq_quest_body.tpl')
			);
	...
	if($fp)
	{
		...
		{
			message_die(GENERAL_ERROR, 'flock() failed', '', __LINE__, __FILE__);
		}
	}
	else
	{
...
$template->set_filenames(array(
	'body' => 'admin/lang_faq_editor_body.tpl')
);
