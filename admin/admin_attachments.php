...
@include_once($phpbb_root_path . 'mods/attachments/includes/constants.'.$phpEx);
include($phpbb_root_path . 'includes/functions_admin.'.$phpEx);
@include_once($phpbb_root_path . 'mods/attachments/includes/functions_attach.'.$phpEx);
...
include($phpbb_root_path . 'mods/attachments/includes/functions_selects.' . $phpEx);
...
// Remove cache file
@unlink($phpbb_root_path . 'cache/config_attach.'.$phpEx);
...
if ($mode == 'manage')
{
	...
	$template->assign_vars(array(
		...
		'L_ATTACHMENT_SETTINGS' => $lang['Attachment'] . ' ' . $lang['Setting'],
...
if ($mode == 'shadow')
{
	...
	$shadow_attachments = $shadow_row = array();
	...
	$table_attachments = $assign_attachments = $file_attachments = array();
...
if ($mode == 'cats')
{
	...
	$s_assigned_group_images = $s_assigned_group_streams = $s_assigned_group_flash = array();
...
if ($mode == 'sync')
{
	...
	echo $attach_menu . $post_menu . '
		</ul>
	</div></td>
	<td valign="top" width="78%">
	<h1>' . $lang['Sync_attachments'] . ' ' . $lang['Attachments'] . '</h1>
	<p></p>
	<table width="100%" cellpadding="4" cellspacing="1" class="forumline" align="center">
	<tr>
		<th class="thHead">' . ((isset($lang['Sync_topics'])) ? $lang['Sync_topics'] : 'Sync Topics') . '</th>
	</tr>
	<tr>
		<td class="row2" height="30">';
	...
	echo '</td>
		...
		<tr>
			<th class="thHead">' . (isset($lang['Sync_posts']) ? $lang['Sync_posts'] : 'Sync Posts');
	...
	echo '</td>
		...
		<tr>
			<th class="thHead">' . (isset($lang['Sync_thumbnails']) ? $lang['Sync_thumbnails'] : 'Sync Thumbnails');
	...
	die('</td>
		...
		<tr>
			<td class="row1" height="30" align="center"><span class="gen">' . $lang['Attach_sync_finished'] . (($info) ? '<br /><br />' . $info : '') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid('index.'.$phpEx) . '">', '</a>') . '</span></td>
		</tr>
		</table>');
