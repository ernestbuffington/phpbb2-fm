...
if ($view == 'stats')
{
	...
	$template->assign_vars(array(
		...
		'L_VERSION' => $lang['Attachment'] . ' ' . $lang['Version'],
		...
		'VERSION' => $attach_config['attach_version'])
	);
...
if ($view == 'username')
{
	...
	$sql = "SELECT u.username, a.user_id_1 as user_id, u.user_level, COUNT(*) AS total_attachments
	...
	if ( $num_members > 0 )
	{
		...
		{
			$username = username_level_color($members[$i]['username'], $members[$i]['user_level'], $members[$i]['user_id']);
