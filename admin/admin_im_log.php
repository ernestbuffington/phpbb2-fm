...
if( $mode == 'edit' && ( isset($_REQUEST['username']) || isset($_REQUEST[POST_USERS_URL]) ) )
{
	...
	$template->set_filenames(array(
		'body' => 'admin/prill_log_body.tpl')
	);

	// Define basic or common template variables
	$template->assign_vars(array(
		'L_ML_TITLE' => $lang['Prillian_Log'],
	...
	if( $msgs_total )
	{
		$s_hidden_fields = '<input type="hidden" name="sid" value="' . $userdata['session_id'] . '" /><input type="hidden" name="mode" value="' . $mode . '" /><input type="hidden" name="' . POST_USERS_URL . '" value="' . $userid . '" />';
...
else if ( $mode == 'lookup' )
{
	...
	if ( !empty($sql_where) )
	{
		...
		else
		{
			...
			$template->set_filenames(array(
				'body' => 'admin/prill_lookup_body.tpl')
			);

			$template->assign_vars(array(
				'L_USER_TITLE' => $lang['Prillian_Log'],
