...
*
* @package admin
* @version $Id: admin_priv_msgs.php,v 1.3 2005/08/02 23:54:41 Nivisec Exp $
...
$aprvmUtil->copyrightYear = '2001, ' . date('Y');
...
if (!empty($setmodules))
{
	...
	$module['General']['Private_Messages'] = $filename;
	return;
}
...
$page_title = $lang['Manage_Private_Messages'];
...
$pmtypes = array(PRIVMSGS_ALL_MAIL, PRIVMSGS_READ_MAIL, PRIVMSGS_NEW_MAIL, PRIVMSGS_SENT_MAIL, PRIVMSGS_SAVED_IN_MAIL, PRIVMSGS_SAVED_OUT_MAIL, PRIVMSGS_UNREAD_MAIL, PRIVMSGS_REPLY_MAIL);
/*
...
switch($pmaction)
{
	case 'view_message':
	{
		...
		$template->set_filenames(array(
			'viewmsg_body' => 'admin/priv_msgs_view_body.tpl')
		);
		...
		if ($pmaction == 'remove_old')
		{
			...
			$sql = 'SELECT privmsgs_id FROM '. PRIVMSGS_TABLE ."$archive_text
 				WHERE privmsgs_to_userid NOT IN ($user_id_sql_list)
            		OR privmsgs_from_userid NOT IN ($user_id_sql_list)";			//print $sql;
			if(!$result = $db->sql_query($sql))
			{
...
class aprvmUtils
{
	...
	function init()
	{
		...
		$this->modName = ($board_config['aprvmArchive'] && $mode == 'archive') ? $lang['Manage_Private_Messages'] . ' (' . $lang['Archive'] . ')' : $lang['Manage_Private_Messages'];
		$this->setupConfig();
		$this->makeURLStart();
...
//				$status_message .= sprintf($lang['Inserted_Default_Value'], $configLangs[$HTTP_GET_VARS['config_name']]);
		...
		if ($board_config['aprvmArchive'])
		{
			$sql = 'SELECT privmsgs_id 
				FROM ' . PRIVMSGS_TABLE .$this->archiveText;
			if(!$result = $db->sql_query($sql))
			{
	...
	function id_2_name($id, $mode = 'user')
	{
		...
		switch($mode)
		{
			case 'user':
			{
				...
				return username_level_color($row['username'], $row['user_level'], $id);
				break;
			}
			case 'reverse':
			{
	...
	*/
	function copyright()
	{
		printf('<br /><div align="center" class="copyright">%s %s &copy; %s <a href="http://www.nivisec.com" class="copyright" target="_blank">Nivisec.com</a>', $this->modName, $this->modVersion, $this->copyrightYear);
		printf('</div>');
	}
		...
		foreach($this->deleteQueue as $post_id)
		{
			if ($board_config['aprvmArchive'] && isset($HTTP_POST_VARS['archive_id_' . $post_id]))
			{
				...
				$sql = 'SELECT privmsgs_subject 
					FROM ' . PRIVMSGS_TABLE . $aprvmUtil->archiveText . " 
			...
			$sql = 'SELECT privmsgs_subject, privmsgs_to_userid, privmsgs_type 
				FROM ' . PRIVMSGS_TABLE . $aprvmUtil->inArchiveText."
			   	WHERE privmsgs_id IN ($postList)";
			if(!$result = $db->sql_query($sql))
			{
				message_die(GENERAL_ERROR, $lang['Error_Posts_Table'], '', __LINE__, __FILE__, $sql);
