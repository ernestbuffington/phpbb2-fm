	...
	if ( !empty($search_match) )
	{
		...
		if ( $row = $db->sql_fetchrow($result) )
		{
			...
			do
			{
				...
				$username_list .= '<tr>
						<td class="' . $row_class . '"><a href="' . append_sid('admin_users.'.$phpEx.'?mode=edit&amp;' . POST_USERS_URL . '=' . $row['user_id']) . '">' . username_level_color($row['username'], $row['user_level'], $row['user_id']) . '</a></td>
