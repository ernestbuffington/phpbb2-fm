...
*
* @package admin
* @version $Id: admin_ratings.php v1.1.0 2003/05/19 17:49:34 gentle_giant Exp $
...
//	$module['Forums']['Rating_System'] = $file;
	return;
}
...
if (isset($HTTP_POST_VARS['rating_form_submitted']))
{
	...
	{
		message_die(GENERAL_ERROR, 'Hacking attempt!');
	}
	...
	if ($HTTP_POST_VARS['submit'])
	{
		...
		$sql = 'SELECT config_id, label, input_type 
			FROM ' . RATING_CONFIG_TABLE . ' 
			ORDER BY config_id';
		...
		if (sizeof($r_error)==0)
		{
			while (list($id, $val) = each($config))
			{
				...
				if( !$result = $db->sql_query($sql) )
				{
					message_die(GENERAL_ERROR, "Could not update rating configuration information", "", __LINE__, __FILE__, $sql);
				}
				...
				if ( $id == 2 && $firstpostonly != $val )
				{
					...
					$sql = 'SELECT t.topic_id 
						FROM ' . RATING_TABLE . ' r, ' . POSTS_TABLE . ' p, ' . TOPICS_TABLE . ' t 
					...
					if( !$result = $db->sql_query($sql) )
					{
						message_die(GENERAL_ERROR, "Could not get rating information", "", __LINE__, __FILE__, $sql);
					}
	...
	elseif ($HTTP_POST_VARS['r_submit'])
	{
		...
		if ( empty($r_error) )
		{
			...
			{
				// ADD
				$sql = 'INSERT INTO ' . RATING_OPTION_TABLE . ' (points, label, weighting, user_type)
					VALUES (' . $points . ', "' . $label . '", ' . $threshold . ', ' . $who . ')';
				if( !$result = $db->sql_query($sql) )
				{
					message_die(GENERAL_ERROR, 'Could not add rating option', '', __LINE__, __FILE__, $sql);
				}
			}
			else
			{
				// UPDATE
				$sql = 'UPDATE ' . RATING_OPTION_TABLE . ' 
					SET points = ' . $points . ', label = "' . $label . '", weighting = ' . $threshold . ', user_type = ' . $who . ' 
					WHERE option_id = ' . $key;
				if( !$result = $db->sql_query($sql) )
				{
					message_die(GENERAL_ERROR, 'Could not update rating option', '', __LINE__, __FILE__, $sql);
				}
	...
	elseif ($HTTP_POST_VARS['r_delete'])
	{
		...
		if ( $used > 0 )
		{
			...
			if(!$result = $db->sql_query($sql))
			{
				message_die(GENERAL_ERROR, 'Could not delete ratings', '', __LINE__, __FILE__, $sql);
			}
			...
			if(!$result = $db->sql_query($sql))
			{
				message_die(GENERAL_ERROR, 'Could not delete bias settings', '', __LINE__, __FILE__, $sql);
			}
		}

		$sql = 'DELETE FROM '.RATING_OPTION_TABLE.' 
			WHERE option_id = '.$key;
		if(!$result = $db->sql_query($sql))
		{
			message_die(GENERAL_ERROR, 'Could not delete rating option', '', __LINE__, __FILE__, $sql);
		}
	...
	elseif ($HTTP_POST_VARS['t_submit'])
	{
		...
		if (empty($t_error))
		{
			...
			{
				// ADD
				$sql = 'INSERT INTO '.RATING_RANK_TABLE.' (type, average_threshold, sum_threshold, label, icon, user_rank) 
					VALUES ('.$type.', '.$average.', '.$sum.', "'.$label.'", "'.$icon.'", '.$urank.')';
				if(!$result = $db->sql_query($sql))
				{
					message_die(GENERAL_ERROR, 'Could not add rating total', '', __LINE__, __FILE__, $sql);
				}
			}
			else
			{
				// CHECK EXISTING VALUES
				$sql = 'SELECT type, average_threshold, sum_threshold 
					FROM '.RATING_RANK_TABLE.' 
					WHERE rating_rank_id = '.$key;
				if(!$result = $db->sql_query($sql))
				{
					message_die(GENERAL_ERROR, 'Could not check rating total information', '', __LINE__, __FILE__, $sql);
				}
				...
				// UPDATE
				$sql = 'UPDATE '.RATING_RANK_TABLE.' 
					SET type = '.$type.', average_threshold = '.$average.', sum_threshold = '.$sum.', label = "'.$label.'", icon = "'.$icon.'", user_rank = '.$urank.' 
					WHERE rating_rank_id = '.$key;
				if(!$result = $db->sql_query($sql))
				{
					message_die(GENERAL_ERROR, 'Could not update rating total', '', __LINE__, __FILE__, $sql);
				}
	...
	elseif ($HTTP_POST_VARS['t_delete'])
	{
		...
		if ( $type == 5 )
		{
			...
			{
				message_die(GENERAL_ERROR, 'Could not get post data', '', __LINE__, __FILE__, $usql);
			}
			$postlist = array();
		...
		elseif ( $type == 4 )
		{
			$usql = 'SELECT topic_id 
				FROM '.TOPICS_TABLE.' 
				WHERE rating_rank_id = '.$key;
			if(!$uresult = $db->sql_query($usql))
			{
				message_die(GENERAL_ERROR, 'Could not get topic data', '', __LINE__, __FILE__, $usql);
			}
			$topiclist = array();
			while( $urow = $db->sql_fetchrow($uresult) )
			{
		...
		elseif ( $type == 3 )
		{
			$usql = 'SELECT user_id 
				FROM '.USERS_TABLE.' 
				WHERE user_rank = '.$r_total_6[$key];
			if(!$uresult = $db->sql_query($usql))
			{
				message_die(GENERAL_ERROR, 'Could not get user data', '', __LINE__, __FILE__, $usql);
			}
			$userlist = array();
			while( $urow = $db->sql_fetchrow($uresult) )
			{
		...
		$sql = 'DELETE FROM '.RATING_RANK_TABLE.' 
			WHERE rating_rank_id = '.$key;
		if(!$result = $db->sql_query($sql))
		{
			message_die(GENERAL_ERROR, 'Could not delete rating total', '', __LINE__, __FILE__, $sql);
		}
	}
...
$template->set_filenames(array(
	...
	$sql = 'SELECT config_id, label, num_value, text_value, input_type 
		FROM '.RATING_CONFIG_TABLE.' 
		ORDER BY list_order';
	if(!$result = $db->sql_query($sql))
	{
...
//					break;
			...
			switch ($row['input_type'])
			{
				case 2:
				// NUMBER
					$input .= '<input class="post" type="text" value="'.$row['num_value'].'" size="5"';
					break;
				case 3:
				// DROPDOWN, YES/NO UNLESS OTHERWISE SPECIFIED
					...
					while (list($key, $val) = each($select_options))
					{
						...
						$option .= ($key == $thisval) ? ' selected="selected">' : '>';
						$option .= $val.'</option>';
				...
				default:
				// TEXTBOX
					$input .= '<input class="post" type="text" value="'.$row['text_value'].'"';
			}
...
$sql = 'SELECT option_id, points, label, weighting, user_type 
	FROM '.RATING_OPTION_TABLE.' 
	ORDER BY points DESC';
if(!$result = $db->sql_query($sql))
{
	message_die(GENERAL_ERROR, "Could not query rating system information", "", __LINE__, __FILE__, $sql);
}
else
{
	// GET TOTAL TIMES EACH OPTION HAS BEEN USED TO DATE
	$sql2 = 'SELECT option_id, COUNT(*) AS num 
		FROM '.RATING_TABLE.' GROUP BY option_id';
	...
	while( $row = $db->sql_fetchrow($result) )
	{
		...
		{
			$delete = $option_submit = '';
		}
		else
		{
			$delete = '<input type="submit" class="liteoption" name="r_delete['.$id.']" value="'.$lang['Rating_delete'].'"';
			$delete .= ( $used > 0 ) ? ' onclick = "return confirm(\''.$lang['Rating_remove_confirm'].'\')">' : '>';
			$option_submit =  '<input type="submit" name="r_submit['.$id.']" value="'.$lang['Rating_update'].'" class="liteoption" />';
		}
		...
		{
			$selected = ($key == $user_type) ? 'selected="selected"' : '';
			$template->assign_block_vars('option.who', array(
				'ID' => $key,
				'SELECTED' => $selected,
				'LABEL' => $val)
	...
	$option_submit = ( $rating_view_only == 1 ) ? '' : '<input type="submit" name="r_submit[0]" value="Add" class="mainoption" />';
		...
		while (list($key,$val) = each($user_types))
		{
			$selected = ($key == $r_option_4[0]) ? 'selected="selected"' : '';
			$template->assign_block_vars('option.who', array(
				'ID' => $key,
				'SELECTED' => $selected,
				'LABEL' => $val)
	...
	if(!$result = $db->sql_query($sql))
	{
		message_die(GENERAL_ERROR, "Could not query rating total information", "", __LINE__, __FILE__, $sql);
	}
	else
	{
		// EACH EXISTING RATING ENTRY
		while( $row = $db->sql_fetchrow($result) )
		{
			...
			{
				$total_submit = '<input type="submit" name="t_submit['.$id.']" value="Update" class="liteoption">';
				$delete = '<input type="submit" name="t_delete['.$id.']" value="Delete" onclick = "return confirm(\''.$lang['Rating_recalc_confirm'].'\')" class="liteoption" />';
			}
			...
			{
				$selected = ($key == $total_type) ? 'selected="selected"' : '';
		...
		$total_submit = ( $rating_view_only == 1 ) ? '' : '<input type="submit" name="t_submit[0]" value="'.$lang['Rating_add'].'" class="mainoption" />';
		...
		while (list($key,$val) = each($total_types))
		{
			$selected = ($key == $r_total_1[0]) ? 'selected="selected"' : '';
			$template->assign_block_vars('total.type', array(
				'ID' => $key,
				'SELECTED' => $selected,
				'LABEL' => $val)
	...
	if(!$result = $db->sql_query($sql))
	{
		message_die(GENERAL_ERROR, "Could not query user rank information", "", __LINE__, __FILE__, $sql);
	}
	else
	{
		// EACH EXISTING RATING ENTRY
	...
	$sql = 'SELECT rating_rank_id, user_rank, average_threshold, sum_threshold 
		FROM '.RATING_RANK_TABLE.' 
		WHERE type = 3
	 	ORDER BY type, sum_threshold DESC, average_threshold DESC';
	if(!$result = $db->sql_query($sql))
	...
	else
	{
		...
		while( $row = $db->sql_fetchrow($result) )
		{
			...
			{
				$selected = ($key == $row['user_rank']) ? 'selected="selected"' : '';
	...
	$config_submit = ($rating_view_only == 1) ? '' : '<input type="submit" name="submit" value="'.$lang['Rating_update_config'].'" class="mainoption" />';
	...
	$template->assign_vars(array(
		...
		'HEADING' => $lang['Rating_System'] . ' ' . $lang['Setting'],
