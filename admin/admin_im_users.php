...
if( !empty($setmodules) )
{
	...
	$module['Prillian']['User_Administration'] = $filename;
	return;
}
...
if( $mode == 'edit' || $mode == 'save' && isset($_REQUEST['submit']) && (  isset($_REQUEST['username']) || isset($_REQUEST[POST_USERS_URL]) ) )
{
	...
	if( isset( $_REQUEST['submit'] ) )
	{
		...
		{
			message_die(GENERAL_ERROR, 'Could not update preferences table', '', __LINE__, __FILE__, $sql);
		}

		$message = $lang['Admin_user_updated'] . '<br /><br />' . sprintf($lang['Click_return_useradmin'], '<a href="' . append_sid("admin_im_users.$phpEx") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . append_sid("index.$phpEx?pane=right") . '">', '</a>');
	...
	$s_hidden_fields .= '<input type="hidden" name="' . POST_USERS_URL . '" value="' . $this_userdata['user_id'] . '" />';
	...
	$template->set_filenames(array(
		'body' => 'admin/prill_user_body.tpl')
	);
	...
	$template->assign_vars(array(
		...
		'OPEN_PMS_YES' => ( $open_pms ) ? 'checked="checked"' : '',
		'OPEN_PMS_NO' => ( !$open_pms ) ? 'checked="checked"' : '',
		'AUTO_DELETE_YES' => ( $auto_delete ) ? 'checked="checked"' : '',
		'AUTO_DELETE_NO' => ( !$auto_delete ) ? 'checked="checked"' : '',
		'ALLOW_NETWORK_YES' => ( $admin_allow_network ) ? 'checked="checked"' : '',
		'ALLOW_NETWORK_NO' => ( !$admin_allow_network ) ? 'checked="checked"' : '',
		'ALLOW_IMS_YES' => ( $admin_allow_ims ) ? 'checked="checked"' : '',
		'ALLOW_IMS_NO' => ( !$admin_allow_ims ) ? 'checked="checked"' : '',
		'ALLOW_SHOUT_YES' => ( $admin_allow_shout ) ? 'checked="checked"' : '',
		'ALLOW_SHOUT_NO' => ( !$admin_allow_shout ) ? 'checked="checked"' : '',
		'ALLOW_CHAT_YES' => ( $admin_allow_chat ) ? 'checked="checked"' : '',
		'ALLOW_CHAT_NO' => ( !$admin_allow_chat ) ? 'checked="checked"' : '',
		'ALWAYS_ADD_SIGNATURE_YES' => ( $attach_sig ) ? 'checked="checked"' : '',
		'ALWAYS_ADD_SIGNATURE_NO' => ( !$attach_sig ) ? 'checked="checked"' : '',
		'REFRESH_RATE' => $refresh_rate,
		'REFRESH_METHOD_YES' => ( $refresh_method == 1 ) ? 'checked="checked"' : '',
		'REFRESH_METHOD_NO' => ( !$refresh_method ) ? 'checked="checked"' : '',
		'REFRESH_METHOD_BOTH' => ( $refresh_method == 2) ? 'checked="checked"' : '',
		'AUTO_LAUNCH_YES' => ( $auto_launch ) ? 'checked="checked"' : '',
		'AUTO_LAUNCH_NO' => ( !$auto_launch ) ? 'checked="checked"' : '',
		'POPUP_IMS_YES' => ( $popup_ims ) ? 'checked="checked"' : '',
		'POPUP_IMS_NO' => ( !$popup_ims ) ? 'checked="checked"' : '',
		'LIST_IMS_YES' => ( $list_ims ) ? 'checked="checked"' : '',
		'LIST_IMS_NO' => ( !$list_ims ) ? 'checked="checked"' : '',
		'PLAY_SOUND_YES' => ( $play_sound ) ? 'checked="checked"' : '',
		'PLAY_SOUND_NO' => ( !$play_sound ) ? 'checked="checked"' : '',
		'DEFAULT_SOUND_YES' => ( $default_sound ) ? 'checked="checked"' : '',
		'DEFAULT_SOUND_NO' => ( !$default_sound ) ? 'checked="checked"' : '',
		'SUCCESS_CLOSE_YES' => ( $success_close ) ? 'checked="checked"' : '',
		'SUCCESS_CLOSE_NO' => ( !$success_close ) ? 'checked="checked"' : '',
		'USE_FRAMES_YES' => ( $use_frames ) ? 'checked="checked"' : '',
		'USE_FRAMES_NO' => ( !$use_frames ) ? 'checked="checked"' : '',
...
else if ( $mode == 'lookup' )
{
	...
	if ( !empty($sql_where) )
	{
		...
		{
			$message = $lang['No_user_id_specified'] . '<br /><br />' . sprintf($lang['Click_return_useradmin'], '<a href="' . append_sid("admin_im_users.$phpEx") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . append_sid("index.$phpEx?pane=right") . '">', '</a>');
		...
		else if ( $db->sql_numrows($result) == 1 )
		{
			...
			$message .= $lang['One_user_found'] . '<br /><br />' . sprintf($lang['Click_goto_prefs'], '<a href="' . append_sid("admin_im_users.$phpEx?mode=edit&" . POST_USERS_URL . "=" . $row['user_id']) . '">', '</a>');
...
else
{
	...
	//
	$template->set_filenames(array(
		'body' => 'admin/prill_select_body.tpl')
	);
