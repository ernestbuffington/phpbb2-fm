 ...
 *
* @package admin
* @version $Id: admin_superundo_ug_auth.php,v 1.13.2.9 2005/07/19 20:01:05 acydburn Exp $
...
if( !empty($setmodules) )
{
	...
	$module['Super_mods']['No_perms'] = $filename . "?mode=user";
	return;
}
...
// Start Functions
//
function check_suauth($type, $key, $u_access, $is_admin)
{
	$auth_user = 0;
			...
			else
			{
				...
				while ( $row = $db->sql_fetchrow($result) )
				{
					...
					$template->assign_block_vars('user_row', array(
						...
						'U_USERNAME' => append_sid("admin_superundo_ug_auth.$phpEx?mode=user&" . POST_USERS_URL . "=" . $row['user_id']))
					);
...
$is_user = ( $mode == 'user' ) ? ( ( $ug_info[0]['user_level'] == USER || $ug_info[0]['user_level'] == MOD && $ug_info[0]['user_id'] != ANONYMOUS ) ? 1 : 0 ) : 0;
	...
	for($i = 0; $i < count($forum_access); $i++)
	{
		...
		for($j = 0; $j < count($forum_auth_fields); $j++)
		{
			...
			switch( $value )
			{
				...
				case AUTH_ACL:
					$auth_ug[$forum_id][$key] = ( !empty($auth_access_count[$forum_id]) ) ? check_suauth(AUTH_ACL, $key, $auth_access[$forum_id], $is_admin) : 0;
				...
				case AUTH_MOD:
					$auth_ug[$forum_id][$key] = ( !empty($auth_access_count[$forum_id]) ) ? check_suauth(AUTH_MOD, $key, $auth_access[$forum_id], $is_admin) : 0;
					break;
		...
		// Is user a moderator?
		//
		$auth_ug[$forum_id]['auth_mod'] = ( !empty($auth_access_count[$forum_id]) ) ? check_suauth(AUTH_MOD, 'auth_mod', $auth_access[$forum_id], 0) : 0;
	}
	...
	while( list($forum_id, $user_ary) = @each($auth_ug) )
	{
		...
		$optionlist_mod .= ( $user_ary['auth_mod'] ) ? '<option value="0" selected="selected">'. $lang['Not_Moderator'] . '</option>' : '<option value="0">' . $lang['Not_Moderator'] . '</option>';
	...
	if( count($name) )
	{
		for($i = 0; $i < count($ug_info); $i++)
		{
			...
			if (!$ug_info[$i]['user_pending'])
         	{
				$t_usergroup_list .= ( ( $t_usergroup_list != '' ) ? ', ' : '' ) . '<a href="' . append_sid("admin_superundo_ug_auth.$phpEx?mode=$ug=" . $id[$i]) . '">' . username_level_color($name[$i], $level[$i], $id[$i]) . '</a>';
		}
         	else
         	{
				$t_pending_list .= ( ( $t_pending_list != '' ) ? ', ' : '' ) . '<a href="' . append_sid("admin_superundo_ug_auth.$phpEx?mode=$ug=" . $id[$i]) . '">' . username_level_color($name[$i], $level[$i], $id[$i]) . '</a>';
         	}		
		}
	}
...
else
{
	...
	$template->assign_vars(array(
		...
		'S_HIDDEN_FIELDS' => $s_hidden_fields, 
		'S_' . $l_type . '_ACTION' => append_sid("admin_superundo_ug_auth.$phpEx"))
	);
