...
switch($mode)
{
	case 'autodetect_form':
		$template->set_filenames(array(
			'body' => 'admin/prill_network_detect_body.tpl')
		);
	...
	case 'save':
		...
		if( $new_site )
		{
			...
			$sql = 'INSERT INTO ' . IM_SITES_TABLE . ' (site_name, site_url, site_enable' . $name_sql . ') 
				VALUES (' . "'$site_name', '$site_url', $site_enable $val_sql)";
		}
		else
		{
			$error_msg = 'Could not update network site data';
			$sql = 'UPDATE ' . IM_SITES_TABLE . " 
				SET site_name = '$site_name', site_url = '$site_url', site_phpex = '$site_phpex', site_profile = '$site_profile', site_enable = $site_enable 
	...
	case 'add':
		...
		$template->set_filenames(array(
			'body' => 'admin/prill_network_edit_body.tpl')
		);
	...
	default:
		...
		$template->set_filenames(array(
			'body' => 'admin/prill_network_list_body.tpl')
		);
