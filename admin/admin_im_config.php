...
*
* @package admin
* @version $Id: admin_im_config.php,v  0.4.0 2003/12/23 23:20 acydburn Exp $
...
foreach($admin_defaults as $var=>$param)
{
	...
	if( array_key_exists($var, $prill_config) )
	{
		...
		if( isset($_REQUEST[$var]) )
		{
			...
			$sql[] = 'UPDATE ' . IM_CONFIG_TABLE . " 
				SET config_value = '" . str_replace("\'", "''", $new[$var]) . "' 
	...
	else
	{
		...
		$sql[] = 'INSERT INTO ' . IM_CONFIG_TABLE . ' (config_name, config_value) 
			VALUES (\'' . $var . '\', \'' . str_replace("\'", "''", $new[$var]) . '\')';
	}
}
...
if( isset($_REQUEST['submit']) && !empty($sql) )
{
	...
	// Remove cache file
	@unlink($phpbb_root_path . 'cache/config_prill.'.$phpEx);

	$message = $lang['Board_config_updated'] . '<br /><br />' . sprintf($lang['Click_return_config'], '<a href="' . append_sid("admin_im_config.$phpEx") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . append_sid("index.$phpEx?pane=right") . '">', '</a>');

	message_die(GENERAL_MESSAGE, $message);
}

$template->set_filenames(array(
	'body' => 'admin/prill_config_body.tpl')
);
...
$template->assign_vars(array(
	...
	'L_CONFIGURATION_TITLE' => $lang['Prillian'] . ' ' . $lang['Setting'],
