...
//			attachment_quota_settings('user', $HTTP_POST_VARS['submit'], 'edit');
	...
	else if( !isset( $HTTP_POST_VARS['submit'] ) && $mode != 'save' && !isset( $HTTP_POST_VARS['avatargallery'] ) && !isset( $HTTP_POST_VARS['submitavatar'] ) && !isset( $HTTP_POST_VARS['cancelavatar'] ) )
	{
		...
		$user_lastlogon	= $this_userdata['user_lastlogon'];	
		$user_regdate = $this_userdata['user_regdate'];	
		$user_totaltime = $this_userdata['user_totaltime'];
		$user_session_time = $this_userdata['user_session_time'];
		$user_totallogon = $this_userdata['user_totallogon'];
		$user_totalpages = $this_userdata['user_totalpages'];
	...
	if( isset($HTTP_POST_VARS['avatargallery']) && !$error )
	{
		if( !$error )
		{
			...
			$coppa = ( ( !$HTTP_POST_VARS['coppa'] && !$HTTP_GET_VARS['coppa'] ) || $mode == REGISTER_MODE) ? 0 : TRUE;
	...
	else
	{
		...
		$template->assign_vars(array(
			...
			'TOTAL_ONLINE_TIME' => make_hours($user_totaltime),
			'LAST_ONLINE_TIME' => make_hours($user_session_time - $user_lastlogon),
			'NUMBER_OF_VISIT' => ($user_totallogon > 0) ? $user_totallogon: $lang['None'],
			'NUMBER_OF_PAGES' => ($user_totalpages) ? $user_totalpages: $lang['None'], 
			...
			'L_DAILY_LIMIT_EXPLAIN' => sprintf($lang['Daily_limit_explain'], '<a href="' . append_sid('admin_board.'.$phpEx.'?mode=post') . '" class="gensmall">', '</a>'),
