...
if( isset($_POST['submit']) )
{
	...
	if ( is_array($sql) && (count($sql) > 0) )
	{
		foreach($sql as $do_sql)
		{
			...
			{
				message_die(GENERAL_ERROR, 'Could not update pa_category table.', '', __LINE__, __FILE__, $do_sql);
			}
		}
	}

	$message = $lang['Category_auth_updated'] . '<br /><br />' . sprintf($lang['Click_return_catauth'],  '<a href="' . append_sid("admin_pa_catauth.$phpEx") . '">', '</a>') . "<br /><br />" . sprintf($lang['Click_return_admin_index'], "<a href=\"" . append_sid("index.$phpEx?pane=right") . "\">", "</a>");
