...
*
* @package admin
* @version $Id: admin_stats_edit_module.php,v 4.2.8 2003/03/16 19:38:27 acydburn Exp $
...
if ($submit && $mode == 'mod_edit')
{
	if (isset($HTTP_POST_VARS['update_time']))
	{
		if (intval($mod_info['update_time']) != intval($HTTP_POST_VARS['update_time']))
		{
			$sql = "UPDATE " . MODULES_TABLE . " 
				SET update_time = " . intval($HTTP_POST_VARS['update_time']) . " 
	...
	for ($i = 0; $i < $num_rows; $i++)
	{
		if (isset($HTTP_POST_VARS[trim($rows[$i]['config_name'])]))
		{
			...
			{
				$sql = "UPDATE " . MODULE_ADMIN_TABLE . " 
					SET config_value = '" . trim(htmlspecialchars($HTTP_POST_VARS[trim($rows[$i]['config_name'])])) . "' 
					WHERE config_name = '" . trim($rows[$i]['config_name']) . "' 
...
if (isset($HTTP_POST_VARS['add_group']) && $mode == 'mod_edit')
{
	...
	$sql = "INSERT INTO " . MODULE_GROUP_AUTH_TABLE . " (module_id, group_id) 
...
if ($mode == 'mod_edit')
{
	...
	for ($i = 0; $i < sizeof($module_langs); $i++)
	{
	...
	$sql = "SELECT g.group_id, g.group_name
		...
		WHERE m.group_id = g.group_id 
			AND m.module_id = " . intval($mod_info['module_id']) . "
		ORDER BY group_name";
	if ( !($result = $db->sql_query($sql)) )
	{
		message_die(GENERAL_ERROR, "Couldn't query Groups Table", "", __LINE__, __FILE__, $sql);
	...
	if ( $num_groups > 0 )
	{
		...
		for($i = 0; $i < sizeof($group_name); $i++)
		{
			...
			if (sizeof($added_groups['group_id']) == 0)
			{
				$add = TRUE;
			}
	...
	else
	{
		...
		for($i = 0; $i < sizeof($added_groups['group_id']); $i++)
		{
	...
	$template->assign_vars(array(
		...
		'S_ACTION' => append_sid('admin_stats_edit_module.'.$phpEx.'?mode='.$mode),
	...
	if ($num_rows > 0)
	{
		...
		$language_file = 'modules/language/lang_' . $language . '/lang_modules.' . $phpEx;
	...
	if ( (!isset($HTTP_POST_VARS['fileupload'])) && (!isset($HTTP_POST_VARS['fileselect'])) )
	{
		...
		if (sizeof($module_paks) > 0)
		{
			...
			{
				$selected = ($mod_info['short_name'].'.pak' == $module_paks[$i]) ? ' selected="selected"' : '';
				if ($selected)
				{
					$template->assign_block_vars('switch_pak_select', array());
				}
...
else if ($mode == 'select_module')
{
	...
	if ($db->sql_numrows($result) == 0)
	{
		$message = $lang['No_modules'] . '<br /><br />' . sprintf($lang['Click_install_module'], '<a href="' . append_sid('admin_statistics.'.$phpEx.'?mode=mod_install') . '">', '</a>');
	...
	$template->assign_vars(array(
		...
		'L_EDIT' => $lang['Edit'],
		'S_ACTION' => append_sid('admin_stats_edit_module.'.$phpEx.'?mode=mod_edit'),
