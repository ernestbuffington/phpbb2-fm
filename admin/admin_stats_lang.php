...
*
* @package admin
* @version $Id: admin_stats_lang.php,v 4.2.8 2003/03/16 18:38:29 acydburn Exp $
...
$lang_decollapse = (isset($HTTP_GET_VARS['d_lang'])) ? trim(htmlspecialchars($HTTP_GET_VARS['d_lang'])) : '';
...
{
	$m_mode = ( isset($HTTP_POST_VARS['m_mode']) ) ? htmlspecialchars($HTTP_POST_VARS['m_mode']) : htmlspecialchars($HTTP_GET_VARS['m_mode']);
}
else
{
	$m_mode = '';
...
$add_value = ( isset($HTTP_POST_VARS['add_value']) ) ? trim(htmlspecialchars($HTTP_POST_VARS['add_value'])) : '';

$new_lang_submit = ( isset($HTTP_POST_VARS['new_lang_submit']) ) ? TRUE : FALSE;
$new_language = ( isset($HTTP_POST_VARS['new_language']) ) ? trim(htmlspecialchars($HTTP_POST_VARS['new_language'])) : '';
...
if (($new_lang_submit) && ($new_language != ''))
{
	...
	if (sizeof($installed_languages) > 0)
	{
...
else if (sizeof($delete_complete_lang) > 0)
{
...
else if (($add_key != '') && (sizeof($add_new_key) > 0))
{
	@reset($add_new_key);
...
if ($mode == 'select')
{
	...
	for ($i = 0; $i < sizeof($provided_languages); $i++)
	{
	...
	if ($m_mode == 'edit')
	{
		...
		$language = (isset($HTTP_GET_VARS['lang'])) ? trim(htmlspecialchars($HTTP_GET_VARS['lang'])) : '';
		...
		for ($i = 0; $i < sizeof($current_modules); $i++)
		{
			...
			for ($j = 0; $j < sizeof($lang_entries); $j++)
			{
