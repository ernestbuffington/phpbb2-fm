...
*
* @package phpBB2
* @version $Id: rating.php v1.0.1 2003/03/21 17:49:34 gentle_giantExp $
...
include($phpbb_root_path . 'includes/functions_rating_2.'.$phpEx);
...
if ( empty($p) ) 
{
	message_die(GENERAL_MESSAGE, 'No post id specified'); // NOTE: THIS SHOULD NEVER HAPPEN
}

$i = ( isset($HTTP_GET_VARS['i']) ) ? intval($HTTP_GET_VARS['i']) : intval($HTTP_POST_VARS['i']);
$b = ( isset($HTTP_GET_VARS['b']) ) ? intval($HTTP_GET_VARS['b']) : intval($HTTP_POST_VARS['b']);
$n = ( isset($HTTP_GET_VARS['n']) ) ? intval($HTTP_GET_VARS['n']) : intval($HTTP_POST_VARS['n']);
$o = ( isset($HTTP_GET_VARS['o']) ) ? intval($HTTP_GET_VARS['o']) : intval($HTTP_POST_VARS['o']);
...
if ( isset($HTTP_POST_VARS['rating_form_submitted']) )
{
	...
	if ( (empty($anonymous_old) && !empty($anonymous_new)) || (!empty($anonymous_old) && empty($anonymous_new)) )
	{
		...
		$sql = 'UPDATE ' . USERS_TABLE . ' 
			SET rating_status = ' . $anonymous_rating . ' 
	...
	if ( $current_rating != $new_rating )
	{
		...
		elseif ( $current_rating > 0 ) 
		{
			$sql = 'UPDATE ' . RATING_TABLE . ' 
				SET option_id = ' . $new_rating . ', rating_time = UNIX_TIMESTAMP(NOW()) 
				WHERE user_id = ' . $userdata['user_id'] . ' 
					AND post_id = ' . $p;
		}
		else
		{
			$sql = 'INSERT INTO ' . RATING_TABLE . ' (user_id, post_id, option_id, rating_time) 
				VALUES (' . $userdata['user_id'] . ', ' . $p . ', ' . $new_rating . ', UNIX_TIMESTAMP(NOW()))';
		}
		if( !($result = $db->sql_query($sql)) )
		{
			message_die(GENERAL_ERROR, 'Could not update rating information', '', __LINE__, __FILE__, $sql);
		}

		// GET CURRENT RATINGS
		$sql = 'SELECT p.rating_rank_id AS post_rating, p.poster_id, t.topic_id, t.rating_rank_id AS topic_rating, u.user_rank AS user_rating 
		...
		if( !($result = $db->sql_query($sql)) )
		{
			message_die(GENERAL_ERROR, 'Could not query current rating information', '', __LINE__, __FILE__, $sql);
		}
...
elseif ( !empty($i) || !empty($b) )
{
	...
	$sql = 'SELECT bias_id 
		FROM ' . RATING_BIAS_TABLE . ' 
		WHERE user_id = ' . $userdata['user_id'] . ' 
			AND target_user = ' . $target_user;
	if( !($result = $db->sql_query($sql)) )
	{
		message_die(GENERAL_ERROR, "Could not query current bias information", "", __LINE__, __FILE__, $sql);
	}
	
	if ( !$row = $db->sql_fetchrow($result) )
	{
		// NO BIAS RECORD EXISTS SO CREATE ONE
		$sql = 'INSERT INTO ' . RATING_BIAS_TABLE . ' (user_id, target_user, bias_status, post_id, option_id, bias_time) 
			VALUES (' . $userdata['user_id'] . ', ' . $target_user . ', ' . $new_status . ', ' . $p . ', ' . $o . ', UNIX_TIMESTAMP(NOW()))';
	}
	else
	{
		$sql = 'UPDATE ' . RATING_BIAS_TABLE . ' 
			SET bias_status = ' . $new_status . ', post_id = ' . $p . ', option_id = ' . $o . ', bias_time = UNIX_TIMESTAMP(NOW()) 
			WHERE bias_id = ' . $row['bias_id'];
	}
	if( !($result = $db->sql_query($sql)) )
	{
		message_die(GENERAL_ERROR, 'Could not update bias information', '', __LINE__, __FILE__, $sql);
	}
}
elseif ( !empty($n) )
{
	// MAKE NEUTRAL FOR SPECIFIED USER_ID
	$sql = 'DELETE FROM ' . RATING_BIAS_TABLE . ' 
		WHERE user_id = ' . $userdata['user_id'] . ' 
			AND target_user = ' . $n;
	if( !($result = $db->sql_query($sql)) )
	{
...
if ( $row['auth_view'] > 1 || $row['auth_read'] > 1 )
{
	message_die(GENERAL_MESSAGE, $lang['Die_rate_private']);
}
elseif ( !$userdata['session_logged_in'] && ($row['auth_view'] == 1 || $row['auth_read'] == 1) )
{
...
if( !($result = $db->sql_query($sql)) )
{
	message_die(GENERAL_ERROR, 'Could not query existing rating information', '', __LINE__, __FILE__, $sql);
}
...
else
{
	while( $row = $db->sql_fetchrow($result) )
	{
		...
		{
			$who = ( $row['user_id'] != ANONYMOUS ) ? '<a class="nav" href="'.append_sid($phpbb_root_path.'profile.'.$phpEx.'?mode=viewprofile&amp;' . POST_USERS_URL . '='.$row['user_id']) . '">' . $row['username'] . '</a>' : $lang['Guest'];
		}
		...
		if ( !$userdata['session_logged_in'] || $config_set[11] == 0 || $config_set[6] == 0 || $row['anon'] == 1 || $row['user_id'] == ANONYMOUS )
		{
			...
			// BIAS NOT APPLICABLE
			$buddy_icon = '<img hspace="3" src="templates/' . ( ( $theme['image_cfg'] ) ? $theme['template_name'] . '/images/' . $theme['image_cfg'] : $theme['template_name'] . '/images' ) . '/ratings/buddy_grey.gif" title="' . $bias_message . '" alt="' . $bias_message . '" />';
			$neutral_icon = '<img hspace="3" src="templates/' . ( ( $theme['image_cfg'] ) ? $theme['template_name'] . '/images/' . $theme['image_cfg'] : $theme['template_name'] . '/images' ) . '/ratings/neutral_grey.gif" title="' . $bias_message . '"alt="' . $bias_message . '" />';
			$ignore_icon = '<img src="templates/' . ( ( $theme['image_cfg'] ) ? $theme['template_name'] . '/images/' . $theme['image_cfg'] : $theme['template_name'] . '/images' ) . '/ratings/ignore_grey.gif" title="' . $bias_message . '"alt="' . $bias_message . '" />';
		}
		else
		{
			...
			switch ($bias_status)
			{
				case 1:
					// IGNORED
					$buddy_icon = '<a href="' . $this_url . '&amp;b=' . $row['user_id'] . '&amp;o=' . $row['option_id'] . '" title="' . sprintf($lang['Rating_make_buddy'], $bias_user) . '"><img hspace="3" src="templates/' . ( ( $theme['image_cfg'] ) ? $theme['template_name'] . '/images/' . $theme['image_cfg'] : $theme['template_name'] . '/images' ) . '/ratings/images/buddy_grey.gif" alt="' . sprintf($lang['Rating_make_buddy'], $bias_user) . '" title="' . sprintf($lang['Rating_make_buddy'], $bias_user) . '" /></a>';
					$neutral_icon = '<a href="' . $this_url . '&amp;n=' . $row['user_id'] . '" title="' . sprintf($lang['Rating_make_neutral'], $bias_user) . '"><img hspace="3" src="templates/' . ( ( $theme['image_cfg'] ) ? $theme['template_name'] . '/images/' . $theme['image_cfg'] : $theme['template_name'] . '/images' ) . '/ratings/neutral_grey.gif" alt="' . sprintf($lang['Rating_make_neutral'], $bias_user) . '" title="' . sprintf($lang['Rating_make_neutral'], $bias_user) . '" /></a>';
					$ignore_icon = '<img hspace="3" src="templates/' . ( ( $theme['image_cfg'] ) ? $theme['template_name'] . '/images/' . $theme['image_cfg'] : $theme['template_name'] . '/images' ) . '/ratings/ignore.gif" alt="' . sprintf($lang['Rating_is_ignored'], $bias_user) . '" />';
					break;
				case 2:
					// BUDDY
					$buddy_icon = '<img hspace="3" src="templates/' . ( ( $theme['image_cfg'] ) ? $theme['template_name'] . '/images/' . $theme['image_cfg'] : $theme['template_name'] . '/images' ) . '/ratings/buddy.gif" alt="' . sprintf($lang['Rating_is_buddy'], $bias_user) . '" />';
					$neutral_icon = '<a href="' . $this_url . '&amp;n=' . $row['user_id'] . '" title="' . sprintf($lang['Rating_make_neutral'], $bias_user) . '"><img hspace="3" src="templates/' . ( ( $theme['image_cfg'] ) ? $theme['template_name'] . '/images/' . $theme['image_cfg'] : $theme['template_name'] . '/images' ) . '/ratings/neutral_grey.gif" alt="' . sprintf($lang['Rating_make_neutral'], $bias_user) . '" title="' . sprintf($lang['Rating_make_neutral'], $bias_user) . '" /></a>';
					$ignore_icon = '<a href="' . $this_url . '&amp;i=' . $row['user_id'] . '&amp;o=' . $row['option_id'] . '" title="' . sprintf($lang['Rating_make_ignored'], $bias_user) . '"><img hspace="3" src="templates/' . ( ( $theme['image_cfg'] ) ? $theme['template_name'] . '/images/' . $theme['image_cfg'] : $theme['template_name'] . '/images' ) . '/ratings/ignore_grey.gif" alt="' . sprintf($lang['Rating_make_ignored'], $bias_user) . '" title="' . sprintf($lang['Rating_make_ignored'], $bias_user) . '" />';
					break;
				default:
					// NEUTRAL
					$buddy_icon = '<a href="' . $this_url . '&amp;b=' . $row['user_id'] . '&amp;o=' . $row['option_id'] . '" title="' . sprintf($lang['Rating_make_buddy'], $bias_user) . '"><img hspace="3" src="templates/' . ( ( $theme['image_cfg'] ) ? $theme['template_name'] . '/images/' . $theme['image_cfg'] : $theme['template_name'] . '/images' ) . '/ratings/buddy_grey.gif" alt="' . sprintf($lang['Rating_make_buddy'], $bias_user) . '" title="' . sprintf($lang['Rating_make_buddy'], $bias_user) . '" /></a>';
					$neutral_icon = '<img hspace="3" src="templates/' . ( ( $theme['image_cfg'] ) ? $theme['template_name'] . '/images/' . $theme['image_cfg'] : $theme['template_name'] . '/images' ) . '/ratings/neutral.gif" alt="' . sprintf($lang['Rating_is_neutral'],$bias_user).'" title="'.sprintf($lang['Rating_is_neutral'], $bias_user) . '" />';
					$ignore_icon = '<a href="' . $this_url . '&amp;i=' . $row['user_id'] . '&amp;o=' . $row['option_id'] . '" title="' . sprintf($lang['Rating_make_ignored'], $bias_user) . '"><img hspace="3" src="templates/' . ( ( $theme['image_cfg'] ) ? $theme['template_name'] . '/images/' . $theme['image_cfg'] : $theme['template_name'] . '/images' ) . '/ratings/ignore_grey.gif" alt="' . sprintf($lang['Rating_make_ignored'], $bias_user) . '" title="' . sprintf($lang['Rating_make_ignored'], $bias_user) . '" />';
			}
		}
...
elseif ( empty($rate_post_msg) )
{
	...
	{
		$sql2 = 'SELECT count(*) AS last24hours 
			FROM ' . RATING_TABLE . ' r, ' . POSTS_TABLE . ' p 
			WHERE r.user_id = ' . $userdata['user_id'] . ' 
				AND r.post_id != ' . $p . ' 
				AND r.post_id = p.post_id 
				AND p.poster_id = ' . $poster_id . ' 
				AND r.rating_time > ' . (time() - 86400);
		if( !($result2 = $db->sql_query($sql2)) )
		{
			message_die(GENERAL_ERROR, 'Could not query rating configuration information', '', __LINE__, __FILE__, $sql2);
		}
		else
		{
	...
	if ( empty($rate_post_msg) && $daily_max > 0 )
	{
		$sql2 = 'SELECT COUNT(*) AS last24hours 
			FROM ' . RATING_TABLE . ' 
			WHERE user_id = ' . $userdata['user_id'] . ' 
				AND rating_time > ' . (time() - 86400);
		if( !($result2 = $db->sql_query($sql2)) )
		{
			message_die(GENERAL_ERROR, 'Could not query rating configuration information', '', __LINE__, __FILE__, $sql);
		}
		else
		{
			$row2 = $db->sql_fetchrow($result2);
	...
	{
		$rate_post_msg = $lang['Already_rated_post'];
	}
	...
	{			
		...
		$sql = 'SELECT option_id, points, label 
			FROM ' . RATING_OPTION_TABLE;
		$sql_where = '';
		...
		if( !($result = $db->sql_query($sql)) )
		{
			message_die(GENERAL_ERROR, 'Could not query rating option information', '', __LINE__, __FILE__, $sql);
		}
		elseif( $db->sql_numrows($result) == 0)
		{
		...
		else
		{
			if ( !isset($userdata['rating_status']) )
			{
				$sql2 = 'SELECT rating_status 
					FROM ' . USERS_TABLE . ' 
					WHERE user_id = ' . $userdata['user_id'];
				if( !($result2 = $db->sql_query($sql2)) )
				{
					message_die(GENERAL_ERROR, 'Could not query user information', '', __LINE__, __FILE__, $sql2);
				}
				$row2 = $db->sql_fetchrow($result2);
			...
			else
			{
				...
				while( $row = $db->sql_fetchrow($result) )
				{
					...
					$selected = ( $previous_rating == $row['option_id'] ) ? ' checked="checked"' : '';
				...
				$selected = ( empty($previous_rating) ) ? ' checked="checked"' : '';
				$template->assign_block_vars('option', array(
					'ID' => 0,
					'SELECTED' => $selected,
				...
				$end_of_form = '<input type="submit" name="submit" value="' . $lang['Rate_it'] . '" class="mainoption" />';
				...
				if ( $config_set[7] == 1 )
				{
					...
					{
						$anon_checked = ( $rating_status == 1 ) ? ' checked="checked"' : '';
