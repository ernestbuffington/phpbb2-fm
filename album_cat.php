...
*
* @package phpBB2
* @version $Id: album.php,v 2.0.7 2003/03/15 10:16:56 ngoctu Exp $
...
if ($total_pics > 0)
{
	...
	for ($i = 0; $i < sizeof($picrow); $i += $album_config['cols_per_page'])
	{
		...
		for ($j = $i; $j < ($i + $album_config['cols_per_page']); $j++)
		{
			...
			{
				$pic_poster = '<a href="'. append_sid("profile.$phpEx?mode=viewprofile&amp;". POST_USERS_URL .'='. $picrow[$j]['user_id']) .'" class="gensmall">'. $picrow[$j]['username'] .'</a>';
			}
			...
			$template->assign_block_vars('picrow.pic_detail', array(
				...
				'EDIT' => ( ( $auth_data['edit'] and ($picrow[$j]['pic_user_id'] == $userdata['user_id']) ) || ($auth_data['moderator'] && ($thiscat['cat_edit_level'] != ALBUM_ADMIN) ) || ($userdata['user_level'] == ADMIN) ) ? '<a href="'. append_sid("album_edit.$phpEx?pic_id=". $picrow[$j]['pic_id']) . '"><img src="' . $images['icon_edit'] . '" alt="' . $lang['Edit_pic'] . '" title="' . $lang['Edit_pic'] . '" /></a>' : '',
				'DELETE' => ( ( $auth_data['delete'] and ($picrow[$j]['pic_user_id'] == $userdata['user_id']) ) || ($auth_data['moderator'] && ($thiscat['cat_delete_level'] != ALBUM_ADMIN) ) || ($userdata['user_level'] == ADMIN) ) ? '<a href="'. append_sid("album_delete.$phpEx?pic_id=". $picrow[$j]['pic_id']) . '"><img src="' . $images['topic_mod_delete'] . '" alt="' . $lang['Delete_pic'] . '" title="' . $lang['Delete_pic'] . '" /></a>' : '',
				'MOVE' => ($auth_data['moderator']) ? '<a href="'. append_sid("album_modcp.$phpEx?mode=move&amp;pic_id=". $picrow[$j]['pic_id']) .'"><img src="' . $phpbb_root_path . $images['topic_mod_move'] . '" alt="' . $lang['Move'] . '" title="' . $lang['Move'] . '" /></a>' : '',
				'LOCK' => ($auth_data['moderator']) ? '<a href="'. append_sid("album_modcp.$phpEx?mode=". (($picrow[$j]['pic_lock'] == 0) ? 'lock' : 'unlock') ."&amp;pic_id=". $picrow[$j]['pic_id']) .'">'. (($picrow[$j]['pic_lock'] == 0) ? '<img src="' . $images['topic_mod_lock'] . '" alt="' . $lang['Lock'] . '" title="' . $lang['Lock'] . '" />' : '<img src="' . $images['topic_mod_unlock'] . '" alt="' . $lang['Unlock'] . '" title="' . $lang['Unlock'] . '" />') . '</a>' : '',
...
$waiting = ($tot_unapproved == 0) ? '' : '<div align="center">' . $lang['Not_approved'] . ': <b>' . $tot_unapproved . '</b></div>';
...
$template->assign_vars(array(
	...
	'L_SLIDE_PIC' => $lang['Slide_Cat'],
	
	'S_COLS' => $album_config['cols_per_page'],
	'S_COL_WIDTH' => (100 / $album_config['cols_per_page']) . '%',
		
	'SLIDE_PIC_IMG' => ($album_config['slidepics_per_page']) ? $images['slide_pic'] : 'images/spacer.gif',
	'SLIDE_PIC_IMG_SPACER' => ($album_config['slidepics_per_page']) ? '&nbsp;&nbsp;&nbsp;' : '',
	...
	'WAITING' => ( ($userdata['user_level'] == ADMIN) || ($userdata['user_level'] == LESS_ADMIN) || ($userdata['user_level'] == MOD) ) ? $waiting : '',
	'MODERATORS' => $moderators_list,
	'CAT_TITLE' => $thiscat['cat_title'],
	'U_VIEW_CAT' => append_sid("album_cat.$phpEx?cat_id=$cat_id"),
	'U_UPLOAD_PIC' => append_sid("album_upload.$phpEx?cat_id=$cat_id"),
	'U_SLIDE_PIC' => append_sid("album_slide.$phpEx?cat_id=$cat_id"),
