...
*
* @package phpBB2
* @version $Id: privmsg.php,v 1.96.2.48 2006/03/18 12:33:06 grahamje Exp $
...
$inbox_url = ( $folder != 'inbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=inbox") . '" class="nav">' . $lang['Inbox'] . '</a>' : $lang['Inbox'];
$inbox_link = append_sid("privmsg.$phpEx?folder=inbox");
...
$outbox_url = ( $folder != 'outbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=outbox") . '" class="nav">' . $lang['Outbox'] . '</a>' : $lang['Outbox'];
$outbox_link = append_sid("privmsg.$phpEx?folder=outbox");
...
$sentbox_url = ( $folder != 'sentbox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=sentbox") . '" class="nav">' . $lang['Sent'] . '</a>' : $lang['Sent'];
$sentbox_link = append_sid("privmsg.$phpEx?folder=sentbox");
...
$savebox_url = ( $folder != 'savebox' || $mode != '' ) ? '<a href="' . append_sid("privmsg.$phpEx?folder=savebox") . '" class="nav">' . $lang['Saved'] . '</a>' : $lang['Saved'];
$savebox_link = append_sid("privmsg.$phpEx?folder=savebox");

$export_url =  '<a href="' . append_sid("privmsg_export.$phpEx") . '" class="nav">' . $lang['Export'] . '</a>';
$export_link =  append_sid("privmsg_export.$phpEx");

if ( $userdata['user_level'] == ADMIN )
{
		$template->assign_block_vars('switch_mass_pm', array(	
			'L_MASS_PM' => $lang['Mass_pm'],
			'MASS_PM_IMG' => $images['mass_pm'],
...
else if ( $mode == 'read' )
{
	...
	switch( $folder )
	{
  	...
  	if( $HTTP_GET_VARS['view']=='next' || $HTTP_GET_VARS['view']=='prev' )
  	{
    	...
    	else
    	{
			...
			$message .= '<br /><br />' . sprintf($lang['Click_return_inbox'], '<a href="' . append_sid("privmsg.$phpEx?folder=inbox") . '">', '</a> ') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid("index.$phpEx") . '">', '</a>');
	...
	//
	$sql = "SELECT u.*, u.username AS username_1, u.user_id AS user_id_1, u.user_level AS user_level_1, u2.username AS username_2, u2.user_id AS user_id_2, u2.user_level AS user_level_2, u.user_allow_viewonline AS user_allow_viewonline_1, u2.user_allow_viewonline AS user_allow_viewonline_2, u.user_session_time AS user_session_time_1, u2.user_session_time AS user_session_time_2, pm.*, pmt.privmsgs_bbcode_uid, pmt.privmsgs_text
	...
	$post_icons = array(
		'post_img' => '<a href="' . $post_urls['post'] . '"><img src="' . $images['pm_postmsg'] . '" alt="' . $lang['Post_new_pm'] . '" title="' . $lang['Post_new_pm'] . '" /></a>',
		'post' => '<a href="' . $post_urls['post'] . '">' . $lang['Post_new_pm'] . '</a>',
		'reply_img' => '<a href="' . $post_urls['reply'] . '"><img src="' . $images['pm_replymsg'] . '" alt="' . $lang['Post_reply_pm'] . '" title="' . $lang['Post_reply_pm'] . '" /></a>',
		'reply' => '<a href="' . $post_urls['reply'] . '">' . $lang['Post_reply_pm'] . '</a>',
		'quote_img' => '<a href="' . $post_urls['quote'] . '"><img src="' . $images['pm_quotemsg'] . '" alt="' . $lang['Post_quote_pm'] . '" title="' . $lang['Post_quote_pm'] . '" /></a>',
		'big_quote_img' => '<a href="' . $post_urls['quote'] . '"><img src="' . $images['pm_quotemsg_big'] . '" alt="' . $lang['Post_quote_pm'] . '" title="' . $lang['Post_quote_pm'] . '" /></a>',
		'quote' => '<a href="' . $post_urls['quote'] . '">' . $lang['Post_quote_pm'] . '</a>',
		'edit_img' => '<a href="' . $post_urls['edit'] . '"><img src="' . $images['pm_editmsg'] . '" alt="' . $lang['Edit_pm'] . '" title="' . $lang['Edit_pm'] . '" /></a>',
	...
	{
		$post_img = $post_icons['post_img'];
		$reply_img = $quote_img = $reply = $quote = '';
	...
	else if ( $folder == 'savebox' )
	{
		...
		else
		{
			$post_img = $post_icons['post_img'];
			$post = $post_icons['post'];
			$reply_img = $quote_img = $edit_img = $reply = $quote = $edit = '';
		}
	...
	$template->assign_vars(array(
		...
		'EDIT_PM' => $edit, 
		'QUOTE_PM' => $quote, 
		'PREVIOUS_PM_IMG' => $images['topic_previous'],
		'NEXT_PM_IMG' => $images['topic_next'],
		'REFRESH_PM_IMG' => $images['topic_refresh'],
		'SEARCH_PM_IMG' => '<a href="' . append_sid("search.$phpEx?search_id=newposts") . '"><img src="' . $images['topic_search'] . '" alt="' . $lang['Search_new'] . '" title="' . $lang['Search_new'] . '" /></a>',
		...
		'INBOX_URL' => $inbox_link, 
		'SENTBOX_URL' => $sentbox_link, 
		'OUTBOX_URL' => $outbox_link, 
		'SAVEBOX_URL' => $savebox_link, 
		...
		'L_REPLY' => $lang['Send_a_reply'],
		'L_PREVIEW' => $lang['Preview'],
		'L_QUICK_PM_REPLY' => $lang['Send_a_reply'],
		...
		'L_POST_NEW_TOPIC' => $lang['Post_new_pm'],
	...
	$user_level_from = $privmsg['user_level_1'];
	$username_to = $privmsg['username_2'];
	$user_id_to = $privmsg['user_id_2'];
	$user_level_to = $privmsg['user_level_2'];
	...
	{
		$icq_status_img = $icq_img = '';
	}

	$aim_img = ( $privmsg['user_aim'] ) ? '<a href="aim:goim?screenname=' . str_replace('+', '', $privmsg['user_aim']) . '&amp;message=Hello+Are+you+there?"><img src="http://big.oscar.aol.com/' . $privmsg['user_aim'] . '?on_url='.$images['icon_aim_online'].'&off_url='.$images['icon_aim_offline'].'" alt="' . $lang['AIM'] . '" title="' . $lang['AIM'] . '" /></a>' : '';	

    $msn_img = ( $privmsg['user_msnm'] ) ? '<a href="http://members.msn.com/' . $privmsg['user_msnm'] . '" target="_blank"><img src="' . $images['icon_msnm'] . '" alt="' . $lang['MSNM'] . '" title="' . $lang['MSNM'] . '" /></a>' : '';

	$yim_img = ( $privmsg['user_yim'] ) ? '<a href="ymsgr:sendIM?' . $privmsg['user_yim'] . '&__you+there?"><img src="http://opi.yahoo.com/online?u=' . $privmsg['user_yim'] . '&m=g&t=1" alt="' . $lang['YIM'] . '" title="' . $lang['YIM'] . '" /></a>' : "&nbsp;";
    ...
    $xfi_img = ( $privmsg['user_xfi'] ) ? '<a href="http://www.xfire.com/xf/modules.php?name=XFire&amp;file=profile&amp;uname=' . $privmsg['user_xfi'] . '" target="_blank"><img src="' . $images['icon_xfi'] . '" alt="' . $lang['XFI'] . '" title="' . $lang['XFI'] . '" /></a>' : '';

	// Extra need to go here, Skype, gTalk, stumble
	// ---->
		
	//
	// Online/Offline/Hidden
	//
	if ( $privmsg['user_session_time_1'] >= (time()-($board_config['whosonline_time'] * 60)) )
	{
	...
	if ( $privmsg['user_session_time_2'] >= (time()-($board_config['whosonline_time'] * 60)) )
	{
	...
	if ( $board_config['allow_sig'] && $userdata['user_allowsig'] )
	{
	...
	//
	$private_message = make_addalled2k_link($private_message, $privmsg['privmsgs_id']);

	if ( $privmsg['privmsgs_attach_sig'] && $user_sig != '' )
	{
		$private_message .= '<span class="gensmall"><br /><br />_________________<br />' . make_clickable($user_sig) . '</span>';
	}
	...
	//
	$template->assign_vars(array(
		'MESSAGE_TO' => username_level_color($username_to, $user_level_to, $user_id_to),
		'MESSAGE_REPLY'=> $username,
		'MESSAGE_FROM' => username_level_color($username_from, $user_level_from, $user_id_from),
		'MESSAGE_FROM_QUICK' => $username_from,
...
else if ( $submit || $refresh || $mode != '' )
{
	...
	if ( $submit )
	{
		...
		if ( !empty($HTTP_POST_VARS['username']) )
		{
			...
			{
				$to_usernames .= "'" . htmlspecialchars(trim($name)) . "',";
			}
			$to_usernames[strlen($to_usernames)-1] = " ";
			
			$sql = "SELECT user_id, username, user_notify_pm, user_notify_pm_text, user_email, user_lang, user_active 
				FROM " . USERS_TABLE . "
			...
			while ($to_username_array[$n] && !$error)
			{
				...
				//
				// Has admin prevented user from sending PM to themself?
				//
				if (!$board_config['privmsg_self'])
				{
					if (strcasecmp($to_users[$n]['username'], $userdata['username']) == 0) 
					{ 
						$error = TRUE; 
						$error_msg .= ( ( !empty($error_msg) ) ? '<br />' : '' ) . $lang['Sender_is_recipient'];
					}
				}
				$n++;
			}
		...
		if ( empty($privmsg_subject) )
		{
  		...
  		$HTTP_POST_VARS['message'] = trim($HTTP_POST_VARS['message']);
  		if ( (strlen ($HTTP_POST_VARS['message']) > $board_config['message_maxlength']) && ($board_config['message_maxlength'] > 0) )
        {
  		...
  		if ( (strlen ($HTTP_POST_VARS['message']) < $board_config['message_minlength']) && ($board_config['message_minlength'] > 0) )
        {
		...
		foreach($to_users as $to_userdata)
		{
			...
			if ( $mode != 'edit' )
			{
				...
				if ( $to_userdata['user_notify_pm'] && !empty($to_userdata['user_email']) && $to_userdata['user_active'] )
				{
					...
					$privmsg_text = '';
					if ($to_userdata['user_notify_pm_text'])
					{
						$privmsg_text = "~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n" . $privmsg_subject . "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n" . $message_text;
					}
	...
	else 
	{
		...
		if ( !empty($HTTP_GET_VARS[POST_USERS_URL]) )
		{
			...
			if ( $row = $db->sql_fetchrow($result) )
			{
				...
				$to_user_level = $row['user_level'];
			}
		}
		else if ( $mode == 'edit' )
		{
			$sql = "SELECT pm.*, pmt.privmsgs_bbcode_uid, pmt.privmsgs_text, u.username, u.user_id, u.user_level, u.user_sig 
			...
			$to_user_level = $privmsg['user_level'];
	...
	$ajaxed_preview = '';
	if ( ($board_config['AJAXed_user_list'] || $board_config['AJAXed_pm_preview']) && $board_config['AJAXed_status'] )
	{
		include($phpbb_root_path . 'includes/sajax.'.$phpEx);
		$sajax->request_type = 'POST';
		$sajax->init('ajax.'.$phpEx);
		$sajax->export('preview_pm', 'build_user_list');
		$sajax->handle_client_request();
		$sajax->show_javascript();
		$template->assign_block_vars('switch_ajax', array());
		$template->assign_vars(array('L_SUBMIT' => $lang['Submit'], 'VIEW_TOPIC' => $lang['View_topic'] .' - '));
		$ajaxed_preview = ( $board_config['AJAXed_pm_preview'] ) ? ' onClick="pm(document.post.message.value,document.post.subject.value); return false;"' : '';
	}
	...
	if ( $preview && !$error )
	{
		...
		$template->assign_vars(array(
			...
			'MESSAGE_TO' => username_level_color($to_username, $to_user_level, $to_userid), 
			'MESSAGE_FROM' => username_level_color($userdata['username'], $userdata['user_level'], $userdata['user_id']), 
	...
	$template->assign_vars(array(
		...
		'AJAXED_PREVIEW' => $ajaxed_preview,
		'AJAXED_USER_LIST' => ( $board_config['AJAXed_user_list'] ) ? ' onkeyup="ub(this.value);" autocomplete="off"' : '',
...
// New message
//
$post_new_mesg_url = '<a href="' . append_sid("privmsg.$phpEx?mode=post") . '"><img src="' . $images['post_new'] . '" alt="' . $lang['Send_a_new_message'] . '" title="' . $lang['Send_a_new_message'] . '" /></a>';
...
$sql_tot = "SELECT COUNT(privmsgs_id) AS total 
	FROM " . PRIVMSGS_TABLE . " ";
$sql = "SELECT pm.privmsgs_type, pm.privmsgs_id, pm.privmsgs_date, pm.privmsgs_subject, u.user_id, u.username, u.user_level 
	FROM " . PRIVMSGS_TABLE . " pm, " . USERS_TABLE . " u ";
switch( $folder )
{
	case 'inbox':
...
$post_pm_img = '<a href="' . $post_pm . '"><img src="' . $images['pm_postmsg'] . '" alt="' . $lang['Post_new_pm'] . '" title="' . $lang['Post_new_pm'] . '" /></a>';
...
if ( $row = $db->sql_fetchrow($result) )
{
	...
	do
	{
		...
		{
			$icon_flag = $images['folder_posted'];
			$icon_flag_alt = $lang['Reply_message'];
		}

		$msg_userid = $row['user_id'];
		$msg_username = username_level_color($row['username'], $row['user_level'], $msg_userid);
