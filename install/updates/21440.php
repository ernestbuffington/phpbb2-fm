...
//
$sql = 'ALTER TABLE ' . $table_prefix . 'bank DROP COLUMN `id`';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'bank CHANGE `name` `user_id` MEDIUMINT(8) NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'bank CHANGE `fees` `fees` TINYINT(1) NOT NULL DEFAULT "1"';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'bank DROP INDEX `name`';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'bank ADD PRIMARY KEY (`user_id`)';
_sql($sql, $errored, $error_ary);
...
$sql = 'ALTER TABLE ' . $table_prefix . 'kb_config MODIFY COLUMN `config_name` VARCHAR(255) NOT NULL DEFAULT ""';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'kb_config MODIFY COLUMN `config_value` VARCHAR(255) NOT NULL DEFAULT ""';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'kb_config ADD PRIMARY KEY (`config_name`)'; 
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $table_prefix . 'kb_results MODIFY COLUMN search_array MEDIUMTEXT NOT NULL';
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $table_prefix . 'links CHANGE `link_title` `link_name` TEXT';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'links CHANGE `link_desc` `link_longdesc` TEXT';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'links CHANGE `link_category` `link_catid` MEDIUMINT(8) UNSIGNED DEFAULT "0" NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'links CHANGE `link_joined` `link_time` INT(25) DEFAULT "0"';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'links CHANGE `link_active` `link_approved` TINYINT(1) DEFAULT "0" NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'links CHANGE `user_ip` `poster_ip` VARCHAR(8) NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'links ADD COLUMN `post_username` varchar(25) default NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'links ADD COLUMN `link_pin` int(2) default "0"';
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $table_prefix . 'link_config ADD PRIMARY KEY (`config_name`)'; 
_sql($sql, $errored, $error_ary);

$sql = 'RENAME TABLE ' . $table_prefix . 'links_categories TO ' . $table_prefix . 'link_categories';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'link_categories CHANGE `cat_id` `cat_id` INT(10) NOT NULL AUTO_INCREMENT';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'link_categories CHANGE `cat_title` `cat_name` TEXT DEFAULT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'link_categories CHANGE `cat_order` `cat_order` INT(50)';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'link_categories ADD COLUMN `cat_parent` int(50) default NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'link_categories ADD COLUMN `parents_data` text NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'link_categories ADD COLUMN `cat_links` mediumint(8) NOT NULL default "-1"';
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $table_prefix . 'logs_config ADD PRIMARY KEY (`config_name`)'; 
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $table_prefix . 'lottery CHANGE `user_id` `user_id` INT(10) NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'lottery_history CHANGE `user_id` `user_id` INT(10) NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'lottery_history CHANGE `amount` `amount` INT(10) NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'lottery_history CHANGE `time` `time` INT(10) NOT NULL';
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $table_prefix . 'meeting_data ADD COLUMN `meeting_guest_overall` MEDIUMINT(8) DEFAULT "0" NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'meeting_data ADD COLUMN `meeting_guest_single` MEDIUMINT(8) DEFAULT "0" NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'meeting_data ADD COLUMN `meeting_guest_names` TINYINT(1) DEFAULT "0" NOT NULL';
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $table_prefix . 'meeting_user ADD COLUMN `meeting_guests` MEDIUMINT(8) DEFAULT "0" NOT NULL';
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $table_prefix . 'news ADD PRIMARY KEY (`config_name`)'; 
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $table_prefix . 'pa_config ADD PRIMARY KEY (`config_name`)'; 
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $table_prefix . 'user_shops CHANGE `items_holding` `items_holding` INT(10) UNSIGNED NOT NULL DEFAULT "0"';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'title_infos ADD COLUMN `info_color` VARCHAR(6) DEFAULT "" NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'title_infos ADD COLUMN `title_pos` TINYINT(1) DEFAULT "0" NOT NULL';
_sql($sql, $errored, $error_ary);
...
$sql = 'DELETE FROM ' . $table_prefix . 'config_nav WHERE `url` = "player/index.php"';
_sql($sql, $errored, $error_ary);

$sql = 'UPDATE ' . $table_prefix . 'config_nav SET `url` = "linkdb.php" WHERE `url` = "links.php"';
_sql($sql, $errored, $error_ary);
...
$sql = 'SELECT `cat_id` FROM ' . $table_prefix . 'link_categories ORDER BY cat_order';
$result = _sql($sql, $errored, $error_ary);
if ( $row = $db->sql_fetchrow($result) )
{
	do
	{
		$sql2 = 'SELECT `link_catid` FROM ' . $table_prefix . 'links WHERE `link_approved` = 1 AND `link_catid` = ' . $row['cat_id'];
		$result2 = _sql($sql2, $errored, $error_ary);
					
		$links = $db->sql_fetchrowset($result2);
		$total_links = sizeof($links);

		$sql3 = 'UPDATE ' . $table_prefix . 'link_categories SET `cat_links` = ' . $total_links . ' WHERE `cat_id` = ' . $row['cat_id'];
...
$sql = 'UPDATE ' . $table_prefix . 'link_categories SET `cat_parent` = "0"';
_sql($sql, $errored, $error_ary);
...
$sql = 'UPDATE ' . $table_prefix . 'bank SET `fees` = "1"';
_sql($sql, $errored, $error_ary);

$sql = 'UPDATE ' . $table_prefix . 'link_config SET `config_name` = "settings_link_page" WHERE `config_name` = "linkspp"';
_sql($sql, $errored, $error_ary);
