...
//
$sql = 'ALTER TABLE ' . $prefix . 'auth_access ADD COLUMN `auth_ban` TINYINT(1) DEFAULT "0" NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'auth_access ADD COLUMN `auth_greencard` TINYINT(1) DEFAULT "0" NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'auth_access ADD COLUMN `auth_bluecard` TINYINT(1) DEFAULT "0" NOT NULL';
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $prefix . 'forums ADD COLUMN `auth_ban` TINYINT(2) NOT NULL DEFAULT "3"';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'forums ADD COLUMN `auth_greencard` TINYINT(2) NOT NULL DEFAULT "5"';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'forums ADD COLUMN `auth_bluecard` TINYINT(2) NOT NULL DEFAULT "1"';
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $prefix . 'posts ADD COLUMN `user_avatar` VARCHAR(100)';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'posts ADD COLUMN `user_avatar_type` TINYINT NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'posts ADD COLUMN `rating_rank_id` SMALLINT UNSIGNED NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'posts ADD COLUMN `post_bluecard` TINYINT(1)';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'posts ADD KEY `posts_ratingrankid` (rating_rank_id)';
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $prefix . 'themes ADD COLUMN `adminfontcolor` VARCHAR(6) DEFAULT "FFA34F"';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'themes ADD COLUMN `supermodfontcolor` VARCHAR(6) DEFAULT "009900"';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'themes ADD COLUMN `modfontcolor` VARCHAR(6) DEFAULT "006600"';
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $prefix . 'themes_name ADD COLUMN `adminfontcolor_name` CHAR(50) DEFAULT "Administrator font color"';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'themes_name ADD COLUMN `supermodfontcolor_name` CHAR(50) DEFAULT "Super Moderator font color"';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'themes_name ADD COLUMN `modfontcolor_name` CHAR(50) DEFAULT "Moderator font color"';
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $prefix . 'topics ADD COLUMN `rating_rank_id` SMALLINT UNSIGNED NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'topics ADD KEY `topics_ratingrankid` (rating_rank_id)';
_sql($sql, $errored, $error_ary);
...
//
$sql = 'ALTER TABLE ' . $prefix . 'album ADD COLUMN `pic_desc` TEXT';
_sql($sql, $errored, $error_ary);
	
$sql = 'ALTER TABLE ' . $prefix . 'album_cat ADD COLUMN `cat_view_groups` VARCHAR(255)';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'album_cat ADD COLUMN `cat_upload_groups` VARCHAR(255)';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'album_cat ADD COLUMN `cat_rate_groups` VARCHAR(255)';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'album_cat ADD COLUMN `cat_comment_groups` VARCHAR(255)';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'album_cat ADD COLUMN `cat_edit_groups` VARCHAR(255)';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'album_cat ADD COLUMN `cat_delete_groups` VARCHAR(255)';
_sql($sql, $errored, $error_ary);
				
$sql = 'ALTER TABLE ' . $prefix . 'foing_artists ADD COLUMN `artist_prefix` VARCHAR(16) DEFAULT "" NOT NULL';
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $prefix . 'users ADD COLUMN `user_sound_pm` TINYINT(1) NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'users ADD COLUMN `rating_status` TINYINT UNSIGNED';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'users ADD COLUMN `user_warnings` SMALLINT(5) DEFAULT "0"';			
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'users ADD COLUMN `avatar_sticky` TINYINT NOT NULL';
_sql($sql, $errored, $error_ary);
...
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE '. $prefix .'forums_config (config_name VARCHAR(255) NOT NULL, config_value VARCHAR(255) NOT NULL, PRIMARY KEY (config_name)) TYPE=MYISAM';
_sql($sql, $errored, $error_ary);

$sql = 'DROP TABLE IF EXISTS ' . $prefix . 'links';
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $prefix . 'links (link_id MEDIUMINT(8) UNSIGNED NOT NULL auto_increment, link_title VARCHAR(255) DEFAULT "" NOT NULL, link_desc VARCHAR(255) DEFAULT NULL, link_category MEDIUMINT(8) UNSIGNED DEFAULT "0" NOT NULL, link_url VARCHAR(100) DEFAULT "" NOT NULL, link_logo_src VARCHAR(100) DEFAULT NULL, link_joined int(11) DEFAULT "0" NOT NULL, link_active tinyint(1) DEFAULT "0" NOT NULL, link_hits int(10) UNSIGNED DEFAULT "0" NOT NULL, user_id MEDIUMINT(8) DEFAULT "0" NOT NULL, user_ip VARCHAR(8) DEFAULT "" NOT NULL, last_user_ip VARCHAR(8) DEFAULT "" NOT NULL, PRIMARY KEY (link_id)) TYPE=MYISAM';
_sql($sql, $errored, $error_ary);
		
$sql = 'DROP TABLE IF EXISTS ' . $prefix . 'links_categories';
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $prefix . 'links_categories (`cat_id` MEDIUMINT(8) UNSIGNED NOT NULL auto_increment, `cat_title` VARCHAR(100) DEFAULT "" NOT NULL, `cat_order` MEDIUMINT(8) UNSIGNED DEFAULT "0" NOT NULL, PRIMARY KEY (cat_id), KEY cat_order (cat_order)) TYPE=MYISAM';
_sql($sql, $errored, $error_ary);
...
// phpbb_config data
$sql = 'UPADTE ' . $prefix . 'config SET `config_name` = "allow_photo_upload" WHERE `config_name` = "\"allow_photo_upload"';
_sql($sql, $errored, $error_ary);
...
for ( $row = 0; $row < sizeof($links_data); $row++ )
{
	...
	{
		$sql = "INSERT INTO " . $prefix . "links (`link_title`, `link_desc`, `link_category`, `link_url`, `link_logo_src`, `link_joined`, `link_active`, `link_hits`, `user_id`) VALUES ('" . $links_data[$row]['Title'] . "', '" . $links_data[$row]['Desc'] . "', '4', '" . $links_data[$row]['Url']. "', '" . $links_data[$row]['Logo'] . "', '" . time() . "', '1', '0', '2')";
		_sql($sql, $errored, $error_ary);
	}
}
...
{
	$sql = "INSERT INTO " . $prefix . "links_categories (`cat_title`, `cat_order`) VALUES ('" . $link_cat_data[$i] . "', '" . $id . "')";
	_sql($sql, $errored, $error_ary);
	$id++;
}
			
// phpbb_portal data
$sql = "INSERT INTO " . $prefix . "portal (`portal_title`, `portal_use_url`, `portal_forum`, `portal_list_limit`, `portal_navbar_name`, `portal_navbar`) VALUES ('home page', '0', '1', '3', 'Home', '1')";
_sql($sql, $errored, $error_ary);
