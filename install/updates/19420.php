...
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'users MODIFY COLUMN `user_clockformat` VARCHAR(12) DEFAULT "clock001.swf" NOT NULL';
_sql($sql, $errored, $error_ary);
...
$sql = 'ALTER TABLE ' . $table_prefix . 'bookie_bets ADD COLUMN `bet_cat` INT(3) DEFAULT "1" NOT NULL';
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $table_prefix . 'themes ADD COLUMN `style_version` VARCHAR(6)';
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $table_prefix . 'users MODIFY COLUMN `user_sound_pm` TINYINT(1) DEFAULT "0" NOT NULL';
_sql($sql, $errored, $error_ary);
...
//
$sql = 'UPDATE ' . $table_prefix . 'config SET `config_value` = "clock001.swf" WHERE `config_name` = "default_clock"';
_sql($sql, $errored, $error_ary);

$sql = 'UPDATE ' . $table_prefix . 'config SET `config_value` = "3.0.0" WHERE `config_name` = "bookie_version"';
_sql($sql, $errored, $error_ary);

$sql = 'UPDATE ' . $table_prefix . 'users SET `user_clockformat` = "clock001.swf"';
_sql($sql, $errored, $error_ary);
