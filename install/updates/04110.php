...
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $table_prefix . 'users ADD COLUMN `user_points` MEDIUMINT(8) UNSIGNED NOT NULL';
_sql($sql, $errored, $error_ary);
...
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'album_config (`config_name` VARCHAR(255) NOT NULL, `config_value` VARCHAR(255) NOT NULL, PRIMARY KEY (config_name))';
_sql($sql, $errored, $error_ary);

$sql = 'DROP TABLE IF EXISTS ' . $table_prefix . 'album_comment';
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'album_comment (comment_id INT(11) UNSIGNED NOT NULL auto_increment, comment_pic_id int(11) UNSIGNED NOT NULL, comment_user_id MEDIUMINT(8) NOT NULL, comment_user_ip char(8) NOT NULL, comment_time int(11) UNSIGNED NOT NULL, comment_text TEXT, comment_edit_time int(11) UNSIGNED, comment_edit_count smallint(5) UNSIGNED DEFAULT "0" NOT NULL, comment_edit_user_id MEDIUMINT(8), comment_username VARCHAR(32), PRIMARY KEY (comment_id), KEY comment_pic_id(comment_pic_id), KEY comment_user_id(comment_user_id), KEY comment_user_ip(comment_user_ip), KEY comment_time(comment_time))';
_sql($sql, $errored, $error_ary);

$sql = 'DROP TABLE IF EXISTS ' . $table_prefix . 'album_rate';
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'album_rate (rate_pic_id INT(11) UNSIGNED NOT NULL, rate_user_id MEDIUMINT(8) NOT NULL, rate_user_ip char(8) NOT NULL, rate_point tinyint(3) UNSIGNED NOT NULL, KEY rate_pic_id (rate_pic_id), KEY rate_user_ip(rate_user_ip), KEY rate_point(rate_point))';
_sql($sql, $errored, $error_ary);

$sql = 'DROP TABLE IF EXISTS ' . $table_prefix . 'bank';
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'bank (id int UNSIGNED NOT NULL auto_increment, name CHAR(50) NOT NULL, holding int(20) UNSIGNED DEFAULT "0", totalwithdrew int(20) UNSIGNED DEFAULT "0", totaldeposit int(20) UNSIGNED DEFAULT "0", opentime int(20) UNSIGNED NOT NULL, fees char(32) DEFAULT "on" NOT NULL, PRIMARY KEY (id), INDEX(name)) TYPE=MYISAM';
_sql($sql, $errored, $error_ary);

$sql = 'DROP TABLE IF EXISTS ' . $table_prefix . 'foing_config';
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'foing_config (config_name VARCHAR(255) DEFAULT "" NOT NULL, config_value VARCHAR(255) DEFAULT "" NOT NULL, PRIMARY KEY (config_name)) TYPE=MYISAM';
_sql($sql, $errored, $error_ary);
	
$sql = 'DROP TABLE IF EXISTS ' . $table_prefix . 'foing_albums';
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'foing_albums (album_id MEDIUMINT(9) NOT NULL auto_increment, artist_id smallint(6) DEFAULT "0" NOT NULL, album_title VARCHAR(96) DEFAULT "" NOT NULL, album_year int(4) DEFAULT NULL, PRIMARY KEY (album_id)) TYPE=MYISAM';
_sql($sql, $errored, $error_ary);
	
$sql = 'DROP TABLE IF EXISTS ' . $table_prefix . 'foing_artists';
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'foing_artists (artist_id smallint(11) NOT NULL auto_increment, artist_name VARCHAR(64) DEFAULT "" NOT NULL, artist_url VARCHAR(64) DEFAULT "" NOT NULL, PRIMARY KEY (artist_id)) TYPE=MYISAM';
_sql($sql, $errored, $error_ary);
	
$sql = 'DROP TABLE IF EXISTS ' . $table_prefix . 'foing_genres';
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'foing_genres (genre_id int(3) DEFAULT "0" NOT NULL, genre_name VARCHAR(32) DEFAULT "" NOT NULL, PRIMARY KEY (genre_id)) TYPE=MYISAM';
_sql($sql, $errored, $error_ary);
	
$sql = 'DROP TABLE IF EXISTS ' . $table_prefix . 'foing_playlist';
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'foing_playlist (list_id MEDIUMINT(9) NOT NULL auto_increment, user_id MEDIUMINT(9) DEFAULT "0" NOT NULL, list_title VARCHAR(64) DEFAULT "" NOT NULL, list_desc tinytext NOT NULL, list_pub INT(1) DEFAULT "0" NOT NULL, PRIMARY KEY (list_id), KEY list_id (list_id)) TYPE=MYISAM';
_sql($sql, $errored, $error_ary);
		
$sql = 'DROP TABLE IF EXISTS ' . $table_prefix . 'foing_playlist_data';
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'foing_playlist_data (list_id MEDIUMINT(9) DEFAULT "0" NOT NULL, song_id MEDIUMINT(9) DEFAULT "0" NOT NULL)';
_sql($sql, $errored, $error_ary);
	
$sql = 'DROP TABLE IF EXISTS ' . $table_prefix . 'foing_songs';
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'foing_songs (song_id MEDIUMINT(9) NOT NULL auto_increment, artist_id smallint(6) DEFAULT "0" NOT NULL, genre_id int(3) DEFAULT "0" NOT NULL, user_id MEDIUMINT(9) DEFAULT "0" NOT NULL, album_id MEDIUMINT(9) DEFAULT "0" NOT NULL, status_id int(1) DEFAULT "0" NOT NULL, song_title VARCHAR(96) DEFAULT "" NOT NULL, song_url VARCHAR(255) DEFAULT "" NOT NULL, song_quality int(3) DEFAULT "0" NOT NULL, song_size VARCHAR(16) DEFAULT "" NOT NULL, song_length MEDIUMINT(9) DEFAULT "0" NOT NULL, PRIMARY KEY (song_id)) TYPE=MYISAM';
_sql($sql, $errored, $error_ary);

$sql = 'DROP TABLE IF EXISTS '. $table_prefix . 'foing_status';
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'foing_status (status_id int(11) DEFAULT "1" NOT NULL, status_name VARCHAR(32) DEFAULT "" NOT NULL, PRIMARY KEY (status_id), UNIQUE KEY status_id(status_id), KEY status_id_2 (status_id)) TYPE=MYISAM';
_sql($sql, $errored, $error_ary);
	
$sql = 'DROP TABLE IF EXISTS ' . $table_prefix . 'foing_song_desc';
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'foing_song_desc (song_id MEDIUMINT(9) DEFAULT "0" NOT NULL, song_desc mediumtext NOT NULL, PRIMARY KEY (song_id), UNIQUE KEY song_id(song_id), KEY song_id_2 (song_id)) TYPE=MYISAM';
_sql($sql, $errored, $error_ary);
...
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'modules (module_id MEDIUMINT(8) UNSIGNED NOT NULL auto_increment, short_name VARCHAR(30) DEFAULT NULL, update_time MEDIUMINT(8) DEFAULT "0" NOT NULL, module_order MEDIUMINT(8) DEFAULT "0" NOT NULL, active tinyint(2) DEFAULT "0" NOT NULL, perm_all tinyint(2) UNSIGNED DEFAULT "1" NOT NULL, perm_reg tinyint(2) UNSIGNED DEFAULT "1" NOT NULL, perm_mod tinyint(2) UNSIGNED DEFAULT "1" NOT NULL, perm_admin tinyint(2) UNSIGNED DEFAULT "1" NOT NULL, PRIMARY KEY (module_id)) TYPE=MYISAM';
_sql($sql, $errored, $error_ary);
...
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'shop_items (id INT UNSIGNED NOT NULL auto_increment, name CHAR(32) NOT NULL, shop CHAR(32) NOT NULL, sdesc CHAR(80) NOT NULL, ldesc TEXT NOT NULL, cost INT(20) UNSIGNED DEFAULT "100", startprice INT(20) UNSIGNED DEFAULT "100", raise INT(20) UNSIGNED DEFAULT "100", stock TINYINT(3) UNSIGNED DEFAULT "10", startstock TINYINT(3) UNSIGNED DEFAULT "100", maxstock TINYINT(3) UNSIGNED DEFAULT "100", sold INT(5) UNSIGNED DEFAULT "0" NOT NULL, PRIMARY KEY (id), INDEX(name)) TYPE=MYISAM';
_sql($sql, $errored, $error_ary);

$sql = 'DROP TABLE IF EXISTS ' . $table_prefix . 'shop_transactions';
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'shop_transactions (shoptrans_date INT(11) DEFAULT "0" NOT NULL, trans_user VARCHAR(25) DEFAULT "" NOT NULL, trans_item VARCHAR(32) DEFAULT "" NOT NULL, trans_type VARCHAR(255) DEFAULT "" NOT NULL, trans_total MEDIUMINT(8) DEFAULT "0" NOT NULL, PRIMARY KEY(shoptrans_date)) TYPE=MYISAM';
_sql($sql, $errored, $error_ary);
...
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'stats_config (config_name VARCHAR(100) DEFAULT "" NOT NULL, config_value VARCHAR(100) DEFAULT "" NOT NULL, PRIMARY KEY (config_name)) TYPE=MYISAM';
_sql($sql, $errored, $error_ary);

$sql = 'DROP TABLE IF EXISTS ' . $table_prefix . 'subscriptions_list';
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'subscriptions_list (user_id BIGINT, forum BIGINT, thread BIGINT)';
_sql($sql, $errored, $error_ary);

$sql = 'DROP TABLE IF EXISTS ' . $table_prefix . 'transactions';
_sql($sql, $errored, $error_ary);
$sql = 'CREATE TABLE ' . $table_prefix . 'transactions (trans_date int(11) DEFAULT "0" NOT NULL, trans_from VARCHAR(30) DEFAULT "" NOT NULL, trans_to VARCHAR(30) DEFAULT "" NOT NULL, trans_amount MEDIUMINT(8) DEFAULT "0" NOT NULL, PRIMARY KEY (trans_date)) TYPE=MYISAM';
_sql($sql, $errored, $error_ary);
...
$sql = 'UPDATE ' . $table_prefix . 'config SET `config_name` = "birthday_check_day" WHERE `config_name` = "birthday_check_date"'; 
_sql($sql, $errored, $error_ary);
...
{
	$sql = "INSERT INTO " . $table_prefix . "foing_status (`status_id`, `status_name`) VALUES ('" . $id . "', '" . $foing_status_data[$i] . "')";
	_sql($sql, $errored, $error_ary);
	$id++;
}
...
{
	$sql = "INSERT INTO " . $table_prefix . "foing_config (`config_name`, `config_value`) VALUES ('" . $config_name . "', '" . $config_value . "')";
	_sql($sql, $errored, $error_ary);
}
	
// phpbb_foing_albums data
$sql = 'INSERT INTO ' . $table_prefix . 'foing_albums (`album_id`, `artist_id`, `album_title`) VALUES ("0", "-1", "--")';
_sql($sql, $errored, $error_ary);
			
$sql = 'UPDATE ' . $table_prefix . 'foing_albums SET `album_id` = "0" WHERE `album_id` = "1"';
_sql($sql, $errored, $error_ary);
...
{
	$sql = "INSERT INTO " . $table_prefix . "foing_genres (`genre_id`, `genre_name`) VALUES ('" . $genre_id . "', '" . $genre_name . "')";
	_sql($sql, $errored, $error_ary);
}
...
{
	$sql = "INSERT INTO " . $table_prefix . "stats_config (`config_name`, `config_value`) VALUES ('" . $config_name . "', '" . $config_value . "')";
	_sql($sql, $errored, $error_ary);
}
