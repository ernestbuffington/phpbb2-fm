...
$sql = 'DROP TABLE IF EXISTS ' . $prefix . 'sessions_keys'; 
_sql($sql, $errored, $error_ary);
...
$sql = 'ALTER TABLE ' . $prefix . 'themes ADD COLUMN `image_cfg` VARCHAR(100) DEFAULT "" NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'themes ADD COLUMN `theme_header` TINYINT(1) DEFAULT "0" NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'themes ADD COLUMN `theme_footer` TINYINT(1) DEFAULT "0" NOT NULL';
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $prefix . 'themes MODIFY COLUMN `themes_id` MEDIUMINT(8) UNSIGNED NOT NULL DEFAULT "0"';
_sql($sql, $errored, $error_ary);
...
$sql = 'ALTER TABLE ' . $prefix . 'bookie_bets CHANGE `bet cat` `bet_cat` INT(3) DEFAULT "1" NOT NULL';
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $prefix . 'meeting_data ADD COLUMN `meeting_by_user` MEDIUMINT(8) DEFAULT "0" NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'meeting_data ADD COLUMN `meeting_edit_by_user` MEDIUMINT(8) DEFAULT "0" NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'meeting_data ADD COLUMN `meeting_start_value` MEDIUMINT(8) DEFAULT "0" NOT NULL';
_sql($sql, $errored, $error_ary);
$sql = 'ALTER TABLE ' . $prefix . 'meeting_data ADD COLUMN `meeting_recure_value` MEDIUMINT(8) DEFAULT "5" NOT NULL';
_sql($sql, $errored, $error_ary);

$sql = 'ALTER TABLE ' . $prefix . 'portal ADD COLUMN `portal_wallpaper` TINYINT(1) DEFAULT "0"';
_sql($sql, $errored, $error_ary);
...
$sql = "DELETE FROM " . $prefix . "news WHERE `config_name` = 'news_color'";
_sql($sql, $errored, $error_ary);

// phpbb_forums_config data
$sql = "INSERT INTO " . $prefix . "forums_config (`config_name`, `config_value`) VALUES ('forum_module_wallpaper', '0')";
_sql($sql, $errored, $error_ary);
...
//
$sql = 'UPDATE ' . $prefix . 'meeting_data SET `meeting_by_user` = "2"';
_sql($sql, $errored, $error_ary);
			
$sql = 'UPDATE ' . $prefix . 'meeting_data SET `meeting_edit_by_user` = "2"';
_sql($sql, $errored, $error_ary);

$sql = 'UPDATE ' . $prefix . 'config SET `config_value` = "php" WHERE `config_name` = "xs_php"';
_sql($sql, $errored, $error_ary);
