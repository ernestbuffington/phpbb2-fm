...
*
* @package phpbb
* @version $Id: viewforum.php,v 1.139.2.10 2003/03/04 21:02:44 acydburn Exp $
...
//
// If forum is subforum give user a nice error message
//
if( $forum_row['forum_issub'] == FORUM_ISSUB )
{
	$redirect = "index.$phpEx?" . POST_PARENTFORUM_URL . "=$forum_id&" . POST_HIERARCHIE_URL . "=" . ($forum_row['forum_hier_level'] + 1) . "" . ( ( isset($start) ) ? "&start=$start" : '' ); 
...
//
// Registration Date
if (($userdata['user_regdate'] > (time() - ($forum_row['forum_regdate_limit'] * 86400))) && $userdata['user_level'] == USER )
{
	message_die(GENERAL_MESSAGE, sprintf($lang['Forum_regdate_limit_error'], $forum_row['forum_name'], $forum_row['forum_regdate_limit']) . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid('index.'.$phpEx) . '">', '</a>'));
}
// No. Posts
if (($userdata['user_posts'] < $forum_row['forum_enter_limit']) && $userdata['user_level'] == USER )
{
	message_die(GENERAL_MESSAGE, sprintf($lang['Forum_enter_limit_error'], $forum_row['forum_name'], $forum_row['forum_enter_limit']) . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid('index.'.$phpEx) . '">', '</a>'));
}

if ( $forum_row['forum_status'] == FORUM_LOCKED ) 
{ 
	$locked_forum['post_img'] = ( $is_auth['auth_mod'] ) ? $images['post_new'] : $images['post_locked']; 
	$locked_forum['l_post_new_topic'] = ( $is_auth['auth_mod'] ) ? $lang['Post_new_topic'] : $lang['Forum_locked']; 
	$locked_forum['no_topics_msg'] = ( !$is_auth['auth_mod'] ) ? $lang['Forum_locked'] : $lang['No_topics_post_one']; 
} 
else 
{ 
	$locked_forum['post_img'] = $images['post_new']; 
	$locked_forum['l_post_new_topic'] = $lang['Post_new_topic']; 
	$locked_forum['no_topics_msg'] = $lang['No_topics_post_one']; 
} 
...
$sql = "SELECT t.*, u.username, u.user_id, u.user_level, u2.user_id AS user_id2, u2.user_level AS user_level2, u2.username as user2, u2.user_id as id2, p.post_username, p2.post_username AS post_username2, p2.post_time, pt.post_text, pt.bbcode_uid 
...
//
$s_auth_can = ( ( ($is_auth['auth_post'] && $forum_row['forum_status'] != FORUM_LOCKED) OR $is_auth['auth_mod'] ) ? $lang['Rules_post_can'] : $lang['Rules_post_cannot'] ) . '<br />';
$s_auth_can .= ( ( ($is_auth['auth_reply'] && $forum_row['forum_status'] != FORUM_LOCKED) OR $is_auth['auth_mod'] ) ? $lang['Rules_reply_can'] : $lang['Rules_reply_cannot'] ) . '<br />';
$s_auth_can .= ( ( ($is_auth['auth_edit'] && $forum_row['forum_status'] != FORUM_LOCKED) OR $is_auth['auth_mod'] ) ? $lang['Rules_edit_can'] : $lang['Rules_edit_cannot'] ) . '<br />';
$s_auth_can .= ( ( ($is_auth['auth_delete'] && $forum_row['forum_status'] != FORUM_LOCKED) OR $is_auth['auth_mod'] ) ? $lang['Rules_delete_can'] : $lang['Rules_delete_cannot'] ) . '<br />';
$s_auth_can .= ( ( $is_auth['auth_vote'] || $is_auth['auth_mod'] ) ? $lang['Rules_vote_can'] : $lang['Rules_vote_cannot'] ) . '<br />';
...
$row = $db->sql_fetchrow($result);
$cf_title = $row['cat_title'] . ', ' . $forum_row['forum_name'];
...
$template->assign_vars(array(
	...
	'POST_IMG' => $locked_forum['post_img'],
	'COLSPAN' => $colspan,
	...
	'L_POST_NEW_TOPIC' => $locked_forum['l_post_new_topic'], 
...
if( $total_topics )
{
	...
	for($i = 0; $i < $total_topics; $i++)
	{
		...
		$topic_rowset[$i]['user2'] = username_level_color($topic_rowset[$i]['user2'], $topic_rowset[$i]['user_level2'], $topic_rowset[$i]['user_id2']);
...
else
{
	...
	// No topics
	//
	$no_topics_msg = $locked_forum['no_topics_msg'];
	$template->assign_vars(array(
