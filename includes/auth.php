...
*
* @package includes
* @version $Id: auth.php,v 1.37.2.3 2003/02/25 16:02:59 acydburn Exp $
...
/*
	...
	VIEW, READ, POST, REPLY, EDIT, DELETE, STICKY, ANNOUNCE, VOTE, POLLCREATE, SUGGEST_EVENT, BAN, VOTEBAN, GREENCARD, BLUECARD
...
function auth($type, $forum_id, $userdata, $f_access = '')
{
	...
	{
		case AUTH_ALL:
			$a_sql = 'a.auth_view, a.auth_read, a.auth_post, a.auth_reply, a.auth_edit, a.auth_delete, a.auth_sticky, a.auth_announce, a.auth_globalannounce, a.auth_vote, a.auth_pollcreate, a.auth_suggest_event, a.auth_ban, a.auth_voteban, a.auth_greencard, a.auth_bluecard';
			$auth_fields = array('auth_view', 'auth_read', 'auth_post', 'auth_reply', 'auth_edit', 'auth_delete', 'auth_sticky', 'auth_announce', 'auth_globalannounce', 'auth_vote', 'auth_pollcreate', 'auth_suggest_event', 'auth_ban', 'auth_voteban', 'auth_greencard', 'auth_bluecard');
			break;
		...
		case AUTH_ATTACH:
			break;
		case AUTH_SUGGEST_EVENT:
			$a_sql = 'a.auth_suggest_event';
			$auth_fields = array('auth_suggest_event');
			break;
		case AUTH_BAN: 
			$a_sql = 'a.auth_ban'; 
			...
			break; 
		case AUTH_VOTEBAN: 
			$a_sql = 'a.auth_voteban'; 
			$auth_fields = array('auth_voteban'); 
			break; 
		case AUTH_GREENCARD: 
	...
	for($i = 0; $i < count($auth_fields); $i++)
	{
		...
		if ( $forum_id != AUTH_LIST_ALL )
		{
			...
			if (in_array($forum_id, $itemformaccess) && (!in_array("auth_sticky", $auth_fields) && !in_array("auth_announce", $auth_fields) && !in_array("auth_globalannounce", $auth_fields) && !in_array("auth_delete", $auth_fields) && ($userdata['user_level'] == 0)))
			{
				$value = 1;
			}
