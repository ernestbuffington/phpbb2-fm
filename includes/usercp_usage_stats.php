...
*
* @package includes
* @version $Id: usercp_usgae_stats.php 2003/01/22 Chris Lennert Exp $
...
if( !file_exists($phpbb_root_path . 'language/lang_' . $language . '/lang_admin_usage_stats.'.$phpEx) ) { $language = 'english'; } include($phpbb_root_path . 'language/lang_' . $language . '/lang_admin_usage_stats.' . $phpEx);
...
function get_unpruned_post_count($user_id)
{
    ...
    $sql = "SELECT DISTINCT p.poster_id, COUNT( p.poster_id ) AS post_count 
...
function &get_topics_watched_rows($user_id)
{
    ...
    $sql = 'SELECT t.forum_id, w.user_id, count( w.topic_id ) AS watch_count 
    	FROM ' . TOPICS_WATCH_TABLE . ' AS w 
    		INNER JOIN ' . TOPICS_TABLE . " AS t ON w.topic_id = t.topic_id 
    	GROUP BY w.user_id, t.forum_id HAVING ( w.user_id = $user_id ) 
    	ORDER BY t.forum_id";
    if ( !($result = $db->sql_query($sql)) )
    {
...
function &get_forum_categories_rows($show_all_forums)
{
    ...
    if( $show_all_forums )
    {
        $sql = 'SELECT c.cat_id, c.cat_title, f.forum_id, f.forum_name, 0 AS forum_post_count, 0 AS forum_post_pct 
        	FROM ' . FORUMS_TABLE . ' AS f 
        		INNER JOIN ' . CATEGORIES_TABLE . ' AS c ON f.cat_id = c.cat_id 
...
function &get_forum_usage_rows($user_id, $user_posts, $show_all_forums)
{
    ...
    $sql = "SELECT f.forum_id, f.forum_name, p.poster_id, COUNT(p.poster_id) AS forum_post_count, (COUNT(p.poster_id) / $user_posts)*100 AS forum_post_pct 
    	FROM " . POSTS_TABLE . " AS p 
    		INNER JOIN " . FORUMS_TABLE . " AS f ON f.forum_id = p.forum_id 
...
function &get_forum_topic_starts_rows($user_id)
{
    ...
    $sql = 'SELECT COUNT(topic_id) AS forum_topic_starts, forum_id, topic_poster 
    	FROM  ' . TOPICS_TABLE . " 
    	GROUP BY forum_id, topic_poster HAVING (topic_poster = $user_id)";
    if ( !($result = $db->sql_query($sql)) )
    {
...
elseif( ($viewlevel & BBUS_VIEWLEVEL_MODERATORS) != 0 && $userdata['user_level'] == LESS_ADMIN )
{
    $view_bb_usage_allowed = true;
}
...
if ( $view_bb_usage_allowed )
{
    ...
    $template->set_filenames(array(
    	'usage_stats_template' => 'profile_usage_stats_body.tpl')
    );
    ...
    if ( $forum_usage_rows ) 
    {
        $prscale = (isset($HTTP_POST_VARS['prscale']) ) ? $HTTP_POST_VARS['prscale'] : $board_config[BBUS_CONFIGPROP_PRSCALE_NAME];
        $trscale = (isset($HTTP_POST_VARS['trscale']) ) ? $HTTP_POST_VARS['trscale'] : $board_config[BBUS_CONFIGPROP_TRSCALE_NAME];
        ...
        for ($i = 0; $i < count($forum_usage_rows); $i++)
        {
            ...
            /* The *_per_day calculations assume $memberdays has already
             * been calculated in usercp_viewprofile.php
             */
            ...
            if ( $cur_cat_id != $last_cat_id ) 
            {
                ...
                $u_search = append_sid("search.$phpEx?search_author=" . $profiledata['username'] );
        ...
        $u_scale = append_sid('profile.'.$phpEx.'?mode=viewprofile&amp;' . POST_USERS_URL .'=' . $profiledata['user_id']);
    ...
    $template->assign_vars(array(
        ...
        'URL_COLDESC' => append_sid('profile_usage_stats.' . $phpEx))
    );

    $template->assign_var_from_handle('BB_USAGE_STATS_TEMPLATE', 'usage_stats_template');
}
