<?php
/***************************************************************************
 *   Page Permissions Main Code Module (copyright www.phpBBDoctor.com)
 *
 ...
 *
 ***************************************************************************
 *	Page Permissions Main Code Module (1.0.2)
...
function set_default_page_permissions(&$auth_level, &$disable_page, &$group_list, &$min_post_count, &$max_post_count)
{
	...
	$disable_page = FALSE;
	$group_list = $min_post_count = $max_post_count = 0;
...
$board_page_permissions_file = $phpbb_root_path . 'cache/config_page_perms.'.$phpEx;
...
{
	$row_structure = array('page_id' => 'page_id', 'page_name' => 'page_name', 'disable_page' => 'disable_page', 'page_parm_name' => 'page_parm_name', 'page_parm_value' => 'page_parm_value', 'auth_level' => 'auth_level', 'min_post_count' => 'min_post_count', 'max_post_count' => 'max_post_count', 'group_list' => 'group_list', 'disabled_message' => 'disabled_message');
...
if (in_array($current_page, $page_names))
{
	foreach ($page_names as $key => $value)
	{
		...
		else if (!strpos(' ' . $current_query_string, $check_query_string) === FALSE)
		{
			// we have a winner!
...
if ($userdata['user_level'] <> ADMIN)
{
	...
	switch ( $auth_level )
	{
		...
		case PAGE_AUTH_ADMIN:	// Admin required
			...
			break;
		case PAGE_AUTH_LESS_ADMIN:	// Super Mod required
			if (!($userdata['session_logged_in']))
			{
				redirect(append_sid("login.$phpEx?redirect=" . $current_page . '&' . $current_query_string, true));
			}
