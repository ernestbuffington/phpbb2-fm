...
*
* @package includes
* @version $Id: sessions.php,v 1.58.2.14 2005/05/06 20:50:11 acydburn Exp $
...
function session_begin($user_id, $user_ip, $page_id, $auto_create = 0, $enable_autologin = 0, $admin = 0)
{
	...
	$sql = "UPDATE " . SESSIONS_TABLE . "
		SET session_user_id = $user_id, session_start = $current_time, session_time = $current_time, session_page = $page_id, session_logged_in = $login, session_admin = $admin, is_robot = '$robot'
	...
	if ( !$db->sql_query($sql) || !$db->sql_affectedrows() )
	{
		...
		$sql = "INSERT INTO " . SESSIONS_TABLE . " (session_id, session_user_id, session_start, session_time, session_ip, session_page, session_logged_in, session_admin, is_robot)
			VALUES ('$session_id', $user_id, $current_time, $current_time, '$user_ip', $page_id, $login, $admin, '$robot')";
		if ( !$db->sql_query($sql) )
		{
			$error = TRUE; 
		   	...
		   	{ 
				...
				{ 
			   		if ($row['Rows'] > 2500) 
			   		{ 
						$db->sql_query("DELETE QUICK FROM " . SESSIONS_TABLE . " LIMIT 50"); 
			   		} 
			   		else 
			   		{ 
			...
			if ($error) 	
			{ 
 				$sql2 = "DELETE FROM " . SESSIONS_TABLE . " 
		...
		if ( isset($userdata['user_id']) )
		{
			...
			if ($ip_check_s == $ip_check_u)
			{
				...
				if ( $current_time - $userdata['session_time'] > 60 || ((( $userdata['user_id'] == ANONYMOUS )?$userdata['user_session_topic'] : $userdata['session_topic']) != $thispage_topic) || (( $userdata['user_id'] == ANONYMOUS ) ? $userdata['user_session_page'] : $userdata['session_page']) != $thispage_id )
				{
    				...
    				$sql = "UPDATE " . SESSIONS_TABLE . " 
						SET session_time = $current_time, session_page = $thispage_id, session_topic = '" . $thistopic_id . "'$update_admin
					...
					$sql = "UPDATE " . USERS_TABLE . " 
						SET user_totalpages = user_totalpages + 1, user_totaltime = user_totaltime + ($current_time - " . $userdata['session_time'] . "), user_session_time = $current_time, user_session_page = $thispage_id, user_session_topic = '" . $thistopic_id . "' 
