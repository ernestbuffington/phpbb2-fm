...
*
* @package includes
* @version $Id: usercp_email.php,v 1.7.2.13 2003/06/06 18:02:15 acydburn Exp $
	...
	if ( $row = $db->sql_fetchrow($result) )
	{
		...
		if ( $row['user_viewemail'] || $userdata['user_level'] == ADMIN )
		{
			...
			if ( isset($HTTP_POST_VARS['submit']) )
			{
				...
				if ( !$error )
				{
					...
					if ( $result = $db->sql_query($sql) )
					{
						...
						if ( !empty($HTTP_POST_VARS['read_receipt']) )
   						{
   	                 		$email_headers .= 'Disposition-Notification-To: ' . $userdata['user_email'] . "\n";
   	                 	}
			...
			$template->assign_vars(array(
	            ...
	            'L_READ_RECEIPT' => $lang['Read_receipt'],
