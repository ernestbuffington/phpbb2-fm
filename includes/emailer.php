...
*
* @package includes
* @version $Id: emailer.php,v 1.15.2.34 2003/07/26 11:41:35 acydburn Exp $
...
class emailer
{
	...
	function send($is_html = '', $error_level = 0)
	{
		...
		{
			message_die(CRITICAL_ERROR, 'Could not query board_timezone config value.', '', __LINE__, __FILE__, $sql);
		}
		$row = $db->sql_fetchrow($result); 
		$offset = $row['config_value'];
		
		$rest = substr($offset, 1);
		if (strlen($rest) > 2) 
		{ 
			// eg 3.5
			$stt = strtotime(($offset*(3600)) . ' seconds');
			$sft = date('r', $stt);
			$sft = substr($sft, 0, -6);  // macgyver chomps the +0000
			$sft .= ' ' . $offset{0};
			$rest2 = str_replace('.', '', $rest); // strip the period eg. 3.5 -> 35
			$rest3 = '0' . $rest2 . '0';
		}
		else 
		{
		...
		if ($is_html)
		{
 			$this->extra_headers = (($this->reply_to != '') ? "Reply-to: $this->reply_to\n" : '') . (($this->from != '') ? "From: $this->from\n" : "From: " . $board_config['board_email'] . "\n") . "Return-Path: " . $board_config['board_email'] . "\nMessage-ID: <" . md5(uniqid(time())) . "@" . $board_config['server_name'] . ">\nContent-transfer-encoding: 8bit\nDate: " . date('r', time()) . "\nX-Priority: 3\nX-MSMail-Priority: Normal\nX-Mailer: PHP\nX-MimeOLE: Produced By " . $board_config['sitename'] . "\n" . $this->extra_headers . (($cc != '') ? "Cc: $cc\n" : '')  . (($bcc != '') ? "Bcc: $bcc\n" : '');
		}
		else 
		{
			$this->extra_headers = (($this->reply_to != '') ? "Reply-to: $this->reply_to\n" : '') . (($this->from != '') ? "From: $this->from\n" : "From: " . $board_config['board_email'] . "\n") . "Return-Path: " . $board_config['board_email'] . "\nMessage-ID: <" . md5(uniqid(time())) . "@" . $board_config['server_name'] . ">\nMIME-Version: 1.0\nContent-type: text/plain; charset=" . $this->encoding . "\nContent-transfer-encoding: 8bit\nDate: " . date('r', time()) . "\nX-Priority: 3\nX-MSMail-Priority: Normal\nX-Mailer: PHP\nX-MimeOLE: Produced By Fully Modded phpBB Forum Software\n" . $this->extra_headers . (($cc != '') ? "Cc: $cc\n" : '')  . (($bcc != '') ? "Bcc: $bcc\n" : '');
		}
