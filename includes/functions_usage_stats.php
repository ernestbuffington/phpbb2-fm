...
*
* @package includes
* @version $Id: usercp_usgae_stats.php 2003/01/26 Chris Lennert Exp $
 ...
 ******************************************************************************/
function create_db_property($property_name, $property_value)
{
    ...
    if( !$db->sql_query($sql) )
    {
        message_die(GENERAL_ERROR, "Function create_db_property(): Failed to insert $property_name property into " . CONFIG_TABLE, "", __LINE__, __FILE__, $sql);
    }
...
function set_bb_usage_stats_property($property_name, $property_value)
{
    ...
    $sql = "SELECT config_name, config_value 
    	FROM " . CONFIG_TABLE . " 
    	WHERE config_name = '$property_name'";
    ...
    {
        create_db_property($property_name, $property_value);
    }
    ...
    elseif ( $db_value != $property_value )
    {
        $sql = 'UPDATE ' . CONFIG_TABLE . " 
        	SET config_value = $property_value 
        	WHERE config_name = '" . $property_name . "'";
        if( !$db->sql_query($sql) )
        {
            message_die(GENERAL_ERROR, 'Function set_bb_usage_stats_property(): Failed to update the $property_name row info in the' . CONFIG_TABLE . 'table', '', __LINE__, __FILE__, $sql);
        }
    }
