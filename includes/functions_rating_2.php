...
function update_post_rating($method, $postlist='')
{
	...
	elseif ( $db->sql_numrows($result) > 0 )
	{
		...
		$sql2 = 'UPDATE '.POSTS_TABLE.' 
			SET rating_rank_id = 0 
			WHERE post_id IN '.$zero_list;
	...
	else
	{
		...
		while ( $row = $db->sql_fetchrow($result) )
		{
			...
			{
				$usql = 'UPDATE '.POSTS_TABLE.' 
					SET rating_rank_id = '.$overall_rank.' 
...
function update_user_rating($method, $userlist='')
{
	...
	elseif ( $db->sql_numrows($result) > 0 )
	{
		...
		$sql2 = 'UPDATE '.USERS_TABLE.' 
			SET user_rank = 0 
			WHERE user_id IN '.$zero_list; // SET ACCORDING TO POSTS
		if( !($result2 = $db->sql_query($sql2)) )
		{
			message_die(CRITICAL_ERROR, 'Could not update rating totals', '', __LINE__, __FILE__, $sql2);
		}
	...
	switch($method)
	{
		case 1:
			...
			$sql = 'SELECT '.$field.' 
				...
				WHERE sum_threshold BETWEEN '.$sql_between.' 
					AND type IN ('.$types.') 
				ORDER BY ABS(sum_threshold) DESC, rating_rank_id LIMIT 1';
			break;

		case 2:
			// AVERAGE
			$sql = 'SELECT '.$field.', ABS('.$val.' - average_threshold) AS nearest 
				FROM '.RATING_RANK_TABLE.' 
				WHERE type IN ('.$types.') 
				ORDER BY nearest LIMIT 1';
			break;
		default:
