...
*
* @package phpBB2
* @version $Id: rating_bias.php v1.1.0 2003/06/03 17:49:34 gentle_giantExp $
...
// VALIDATE GET VARS
$rs = intval($HTTP_GET_VARS['rs']);
$m = intval($HTTP_GET_VARS['m']);
$this_url = append_sid($PHP_SELF) . '?m=' . $m . '&rs=' . $rs;
...
if ( $rs == 1 )
{
	...
	$u_alt_screen = append_sid($phpbb_root_path.'rating_bias.'.$phpEx.'?rs=0');
}
else
{
	$page_title = $lang['Rating_my_bias_title'];
	$l_alt_screen = $lang['Rating_their_bias_title'];
	$u_alt_screen = append_sid($phpbb_root_path.'rating_bias.'.$phpEx.'?rs=1');
}
...
{
	message_die(GENERAL_MESSAGE, $lang['Rating_bias_off']); 
}
...
{ 
	...
	{ 
		...
		if ( $s == 1 )
		{
			...
			{
				// IGNORED
				$bias = '<img src="templates/' . ( ( $theme['image_cfg'] ) ? $theme['template_name'] . '/images/' . $theme['image_cfg'] : $theme['template_name'] . '/images' ) . '/ratings/ignore.gif" alt="' . sprintf($lang['Rating_ignored'], $this_user) . '" title="' . sprintf($lang['Rating_ignored'], $this_user).'" />';
			}
			else
			{
				// BUDDY
				$bias = '<img src="templates/' . ( ( $theme['image_cfg'] ) ? $theme['template_name'] . '/images/' . $theme['image_cfg'] : $theme['template_name'] . '/images' ) . '/ratings/buddy.gif" alt="' . sprintf($lang['Rating_buddy'], $this_user) . '" title="' . sprintf($lang['Rating_buddy'], $this_user).'" />';
			}
		}
		// MY BIAS SETTINGS
		else
		{
			$this_user = $row['username'];
			$who = '<a href="' . append_sid($phpbb_root_path . 'profile.' . $phpEx . '?mode=viewprofile&amp;' . POST_USERS_URL . '=' . $row['user_id']) . '">' . $row['username'] . '</a>'; 
			$neutral_icon = '<a href="'.$this_url.'&amp;n='.$row['user_id'].'" title="'.sprintf($lang['Rating_make_neutral'],$row['username']).'"><img src="templates/' . ( ( $theme['image_cfg'] ) ? $theme['template_name'] . '/images/' . $theme['image_cfg'] : $theme['template_name'] . '/images' ) . '/ratings/neutral_grey.gif" alt="' . sprintf($lang['Rating_make_neutral'], $row['username']) . '" title="' . sprintf($lang['Rating_make_neutral'], $row['username']) . '" /></a>';
			if ( $row['bias_status'] == 1 )
			{
				// IGNORED
				$bias = $neutral_icon.'<img src="templates/' . ( ( $theme['image_cfg'] ) ? $theme['template_name'] . '/images/' . $theme['image_cfg'] : $theme['template_name'] . '/images' ) . '/ratings/ignore.gif" alt="' . sprintf($lang['Rating_is_ignored'], $this_user) . '" title="' . sprintf($lang['Rating_is_ignored'], $this_user) . '" />';
			}
			else
			{
				// BUDDY
				$bias = '<img src="templates/' . ( ( $theme['image_cfg'] ) ? $theme['template_name'] . '/images/' . $theme['image_cfg'] : $theme['template_name'] . '/images' ) . '/ratings/buddy.gif" alt="' . sprintf($lang['Rating_is_buddy'], $this_user) . '" title="' . sprintf($lang['Rating_is_buddy'], $this_user) . '" />'.$neutral_icon;
			}
		}
