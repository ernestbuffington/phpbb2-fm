 ...
 *
* @package phpBB2
* @version $Id: profile_mypoints_to.php,v 0.90 2003/031/06 20:00:00 conanqtran Exp $
...
{ 
	redirect("login.".$phpEx."?redirect=profile_mypoints_to.".$phpEx); 
	exit; 
} 
...
//
$mode_types_text = array($lang['Sort_Trans_Date'], $lang['Sort_Trans_From'], $lang['Sort_Trans_Amount'], $lang['Sort_Top_Ten_Trans']);
$mode_types = array('transdate', 'transfrom', 'transamount', 'toptentrans');
...
$template->assign_vars(array(
	...
	'S_MODE_ACTION' => append_sid("profile_mypoints_to.$phpEx"))
);
...
switch( $mode )
{
	case 'transdate':
		...
		break;
	case 'transfrom':
		$order_by = "trans_from $sort_order LIMIT $start, " . $board_config['topics_per_page'];
		break;
	case 'transamount':
...
//
$sql = "SELECT trans_date, trans_from, trans_amount, trans_reason
...
if ( $mode != 'topten' || $board_config['topics_per_page'] < 10 )
{
	...
	if ( $total = $db->sql_fetchrow($result) )
	{
		...
		$pagination = generate_pagination("profile_mypoints_to.$phpEx?mode=$mode&amp;order=$sort_order", $total_transactions, $board_config['topics_per_page'], $start). '&nbsp;';
	}
}
else
{
