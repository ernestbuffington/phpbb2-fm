...
{
	redirect(append_sid("login.$phpEx?redirect=admin_super/index.$phpEx", true));
}
else if ( $userdata['user_level'] != LESS_ADMIN && $userdata['user_level'] != ADMIN )
{
...
if ($board_config['admin_login'])
{
	...
	{
		redirect(append_sid("login.$phpEx?redirect=admin_super/index.$phpEx&admin=1", true));
	}
}
