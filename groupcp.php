...
*
* @package phpBB2
* @version $Id: groupcp.php,v 1.58.2.22 2004/11/18 17:49:34 acydburn Exp $
...
// -------------------------
//
function generate_user_info(&$row, $date_format, $group_mod, &$from, &$posts, &$profile_img, &$profile, &$pm_img, &$pm, &$email_img, &$email, &$www_img, &$www, &$userdata, &$online_status_img, &$username)
{
	global $lang, $images, $board_config, $phpEx;

	$from = ( !empty($row['user_from']) ) ? $row['user_from'] : '&nbsp;';
	$posts = ( $row['user_posts'] ) ? '<a href="' . append_sid("search.$phpEx?search_author=" . urlencode($row['username'])) . '" class="gen">' . $row['user_posts'] . '</a>' : 0;
...
else if ( isset($HTTP_POST_VARS['joingroup']) && $group_id )
{
	...
	else
	{
		...
		$count_group_moderators = sizeof($group_moderators);
...
else if ( $group_id )
{
					...
					if ( isset($HTTP_POST_VARS['approve']) )
					{
						if ( $group_info['auth_mod'] || $userdata['user_level'] == ADMIN )
						{
							...
							$sql = "UPDATE " . USERS_TABLE . " 
								SET user_level = " . MOD . " 
								WHERE user_id IN ($sql_in) 
									AND user_level NOT IN (" . MOD . ", " . LESS_ADMIN . ", " . ADMIN . ")";
							if ( !$db->sql_query($sql) )
							{
								message_die(GENERAL_ERROR, 'Could not update user level', '', __LINE__, __FILE__, $sql);
							}
	...
	//
	$sql = "SELECT username, user_id, user_level, user_viewemail, user_posts, user_regdate, user_from, user_website, user_email, user_icq, user_aim, user_yim, user_msnm, user_allow_viewonline, user_session_time
		FROM " . USERS_TABLE . " 
		WHERE user_id = " . $group_info['group_moderator'];
	if ( !($result = $db->sql_query($sql)) )
	{
		message_die(GENERAL_ERROR, 'Error getting moderator for group', '', __LINE__, __FILE__, $sql);
	}
	...
	//
	$sql = "SELECT u.username, u.user_id, u.user_level, u.user_viewemail, u.user_posts, u.user_regdate, u.user_from, u.user_website, u.user_email, u.user_icq, u.user_aim, u.user_yim, u.user_msnm, u.user_allow_viewonline, u.user_session_time, ug.user_pending, ug.digest_confirm_date
	...
	$sql = "SELECT u.username, u.user_id, u.user_level, u.user_viewemail, u.user_posts, u.user_regdate, u.user_from, u.user_website, u.user_email, u.user_icq, u.user_aim, u.user_yim, u.user_msnm, u.user_allow_viewonline, u.user_session_time
		FROM " . USER_GROUP_TABLE . " ug, " . USERS_TABLE . " u
		WHERE ug.group_id = $group_id
	...
	if ($group_info['group_members_count'])
	{
		$template->assign_block_vars('group_member_count', array());
	}
	...
	else
	{
		...
		else if($group_info['group_type'] == GROUP_PAYMENT )
		{
			...
			$group_details = $lang['L_IPN_Subscribe_Payment_grp'];
			$s_hidden_fields = '';			
		}
	}
	...
	$user_level = $group_moderator['user_level'];
	$user_id = $group_moderator['user_id'];

	generate_user_info($group_moderator, $board_config['default_dateformat'], $is_moderator, $from, $posts, $profile_img, $profile, $pm_img, $pm, $email_img, $email, $www_img, $www, $userdata, $online_status_img, &$username);
	...
	$template->assign_vars(array(
		...
		'L_ONLINE_STATUS' => $lang['View_status'],
		'L_GROUP_OWNER' => $lang['Group_owner'],
		'L_GRANT_UNGRANT_SELECTED' => $lang['Group_grant_mod_status'],

		'AJAXED_USER_LIST' => ( $board_config['AJAXed_user_list'] && $board_config['AJAXed_status'] ) ? ' onkeyup="ub(this.value);" autocomplete="off"' : '',
		'AJAXED_USER_LIST_BOX' => ( $board_config['AJAXed_user_list'] && $board_config['AJAXed_status'] ) ? '<br /><div id="user_list" style="position:absolute"></div>' : '',

		'GROUP_NAME' => $group_info['group_name'],
		'GROUP_DESC' => $group_info['group_description'],
		'GROUP_DETAILS' => $group_details,
    	'GROUP_COUNT' => ( $group_info['group_type'] == GROUP_HIDDEN && !$is_moderator ) ? $lang['Group_hidden_members'] : $members_count + 1,
		'MOD_ROW_COLOR' => '#' . $theme['td_color1'],
		'MOD_ROW_CLASS' => $theme['td_class1'],
		'MOD_USERNAME' => username_level_color($username, $user_level, $user_id),
		'MOD_FROM' => $from,
		'MOD_JOINED' => $joined,
		'MOD_POSTS' => $posts,
	...
	while(!empty($group_members[++$i]['username']))
	{
		...
		$user_level = $group_members[$i]['user_level'];
		$user_id = $group_members[$i]['user_id'];

		generate_user_info($group_members[$i], $board_config['default_dateformat'], $is_moderator, $from, $posts, $profile_img, $profile, $pm_img, $pm, $email_img, $email, $www_img, $www, $userdata, $online_status_img, &$username);

		if ( $group_info['group_type'] != GROUP_HIDDEN || $is_moderator )
		{
			...
			if ( $last_member_type != $group_members[$i]['group_moderator'] )
			{
				$template->assign_block_vars('member_row.member_type', array(
					'L_TYPE' => ( $group_members[$i]['group_moderator'] ) ? ((sizeof($group_members) >= $i + 1 && $group_members[$i + 1]['group_moderator']) ? $lang['Group_Moderators'] : $lang['Group_Moderator']) : $lang['Group_Members'])
				);
	...
	if ( $is_moderator )
	{
		...
		if ( $modgroup_pending_count )
		{
			for($i = 0; $i < $modgroup_pending_count; $i++)
			{
				...
				$user_level = $modgroup_pending_list[$i]['user_level'];
				$user_id = $modgroup_pending_list[$i]['user_id'];

				generate_user_info($modgroup_pending_list[$i], $board_config['default_dateformat'], $is_moderator, $from, $posts, $profile_img, $profile, $pm_img, $pm, $email_img, $email, $www_img, $www, $userdata, $online_status_img, &$username);
